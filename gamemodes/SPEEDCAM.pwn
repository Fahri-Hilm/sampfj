...
stock Speed_Refresh(speedid)
{
	if (speedid != -1 && SpeedData[speedid][speedExists])
	{
		...
		format(string, sizeof(string), "[SPEEDCAM %d]\n{FFFFFF}Speed Limit: %.0f KM/H\nVehicle: %s\nPlate: %s", speedid, SpeedData[speedid][speedLimit], GetVehicleModelName(SpeedData[speedid][speedVehicle]), SpeedData[speedid][speedPlate]);
...
stock Speed_Save(speedid)
{
	...
	format(query, sizeof(query), "UPDATE `speedcameras` SET `speedRange` = '%.4f', `speedLimit` = '%.4f', `speedX` = '%.4f', `speedY` = '%.4f', `speedZ` = '%.4f', `speedAngle` = '%.4f', `speedplate` = '%s', `speedvehicle` = '%d' WHERE `speedID` = '%d'",
 	...
 	if(rows)
  	{
		...
		printf("[SPEEDCAM] Loaded %d Speed Camera from the Database", rows);
	}
	return 1;
}
