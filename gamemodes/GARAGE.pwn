...
{
	new string[8000];
	//for (new garid < 0 || garid >= MAX_GARAGE)
	//if ((id = Garage_Nearest(playerid)) != -1)
	//if (garid != -1 && GarageData[garid][garageExists])
	format(string, sizeof(string), "ID\tName\tFee\tDistance\tLocation\n");
	for (new i = 0; i < MAX_GARAGE; i ++) if (GarageData[i][garageExists])
	{
        format(string, sizeof(string), "%s%d\t%s\t{00FF00}%s\t{FFFFFF}%0.2f Meter\n", string, i, GarageData[i][garageName], FormatMoney(GarageData[i][garageFee]), GetPlayerDistanceFromPoint(playerid, GarageData[i][garagePos][0], GarageData[i][garagePos][1], GarageData[i][garagePos][2]), GetLocation(GarageData[i][garagePos][0], GarageData[i][garagePos][1], GarageData[i][garagePos][2]));
	}
...
CMD:buygarage(playerid, params[])
{
	...
	if ((id = Garage_Nearest(playerid)) != -1)
	{
		...
		if (GarageData[id][garagePrice] > pData[playerid][pMoney])
