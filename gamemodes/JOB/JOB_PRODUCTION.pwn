	...
	if(pData[playerid][pJob] == 6 || pData[playerid][pJob2] == 6)
	{
		...
		|| IsPlayerInRangeOfPoint(playerid, 2.0, -250.88, -2143.23, 29.32) || IsPlayerInRangeOfPoint(playerid, 2.0, -245.74, -2141.65, 29.32))
		{
			...
			if(type == 1)
			{
				...
				pData[playerid][pProducting] = SetTimerEx("CreateProduct", 4000, false, "id", playerid, 1);
				ShowProgressbar(playerid, "Create", 4);
			}
			else if(type == 2)
			{
				if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda masih memiliki activity progress!");
				...
				ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
				pData[playerid][pProductingStatus] = 1;
				pData[playerid][pProducting] = SetTimerEx("CreateProduct", 4000, false, "id", playerid, 2);
				ShowProgressbar(playerid, "Create", 4);
			...
			else if(type == 3)
			{
				...
				ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
				pData[playerid][pProductingStatus] = 1;
				pData[playerid][pProducting] = SetTimerEx("CreateProduct", 4000, false, "id", playerid, 3);
				ShowProgressbar(playerid, "Create", 4);
			}
		}
