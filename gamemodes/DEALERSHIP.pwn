...
GetDealerVehicleCost(carid)
{
	...
	if(carid == 481) return 400;  //Bmx
	if(carid == 509) return 300; //Bike
	if(carid == 510) return 150; //Mt bike
	if(carid == 463) return 10000; //Freeway harley
	if(carid == 521) return 21500; //Fcr 900
	if(carid == 461) return 20300; //Pcj 600
	if(carid == 581) return 18000; //Bf
	if(carid == 468) return 1100; //Sancehz
	if(carid == 586) return 1200; //Wayfare
	if(carid == 462) return 2000; //Faggio
	...
	if(carid == 482) return 15200; //Burrito
	if(carid == 418) return 15800; //Moonbeam
	if(carid == 413) return 15600; //Pony
	...
	if(carid == 420) return 3000; //Taxi
	if(carid == 438) return 4000; //Cabbie
	if(carid == 403) return 8000; //Linerunner
	if(carid == 414) return 4000; //Mule
	if(carid == 422) return 3400; //Bobcat
	if(carid == 440) return 4100; //Rumpo
	if(carid == 455) return 3500; //Flatbead
	if(carid == 456) return 4500; //Yankee
	if(carid == 478) return 3000; //Walton
	if(carid == 498) return 5000; //Boxville
	if(carid == 499) return 3500; //Benson
	if(carid == 514) return 5000; //Tanker
	if(carid == 515) return 6000; //Roadtrain
	if(carid == 524) return 4000; //Cement Truck
	if(carid == 525) return 4500; //Towtruck
	if(carid == 543) return 4200; //Sadler
	if(carid == 552) return 5000; //Utility Van
	if(carid == 554) return 5000; //Yosemite
	if(carid == 578) return 4900; //DFT-30
	if(carid == 609) return 4000; //Boxville
	if(carid == 423) return 4200; //Mr Whoopee/Ice cream
	if(carid == 588) return 4700; //Hotdog
 	return -1;
}
    ...
    if(dialogid == DIALOG_DEALER_VAULT)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				{
					new mstr[512];
					format(mstr,sizeof(mstr),"Uang kamu: %s.\n\nMasukkan berapa banyak uang yang akan kamu simpan di dalam Dealership ini", FormatMoney(pData[playerid][pMoney]));
...
function PlayerDealerUpdate(playerid)
{
