...
Player_OwnsHouser(playerid, houseid)
{
	return(!strcmp(hData[houseid][hOwnern], pData[playerid][pName], true));
}
...
HouseReset(houseid)
{
	...
	format(hData[houseid][hOwnern], MAX_PLAYER_NAME, "-");
...
House_OpenStorage(playerid, houseid)
{
    ...
    if(!Player_OwnsHouse(playerid, houseid) && !Player_OwnsHouser(playerid, houseid))
...
/*GetOwnedHousen(playerid)
{
	new tmpcount;
	foreach(new hid : Houses)
	{
	    if(!strcmp(hData[hid][hOwnern], pData[playerid][pName], true))
	    {
     		tmpcount++;
		}
	}
...
/*ReturnPlayerHousesnID(playerid, hslot)
{
	new tmpcount;
	if(hslot < 1 && hslot > LIMIT_PER_PLAYER) return -1;
	foreach(new hid : Houses)
	{
	    if(!strcmp(pData[playerid][pName], hData[hid][hOwnern], true))
	    {
     		tmpcount++;
       		if(tmpcount == hslot)
       		{
...
{
	new cQuery[1536];
	format(cQuery, sizeof(cQuery), "UPDATE houses SET owner='%s', ownern='%s', ownerid='%d', address='%s', price='%d', type='%d', locked='%d', money='%d'",
	hData[houseid][hOwner],
	hData[houseid][hOwnern],
...
function LoadHouses()
{
	...
	new rows = cache_num_rows(), owner[128], ownern[128], address[128];
 	if(rows)
  	{
		for(new i; i < rows; i++)
		{
			...
			cache_get_value_name(i, "ownern", ownern);
			format(hData[hid][hOwnern], 128, ownern);
...
CMD:edithouse(playerid, params[])
{
    ...
    if(sscanf(params, "ds[24]S()[128]", hid, type, string))
    {
        ...
        SendClientMessage(playerid, COLOR_YELLOW, "[NAMES]:{FFFFFF} location, interior, locked, owner, owner2, ownerid, price, type, reset, delete");
        return 1;
    }
    ...
    {
        new ownersn[MAX_PLAYER_NAME];

        if(sscanf(string, "s[32]", ownersn))
            return Usage(playerid, "/edithouse [id] [owner2] [player name] (use '-' to no owner2)");

        format(hData[hid][hOwnern], MAX_PLAYER_NAME, ownersn);

        House_Save(hid);
		House_Refresh(hid);
        SendAdminMessage(COLOR_RED, "%s has adjusted the owner of house ID: %d to %s", pData[playerid][pAdminname], hid, ownersn);
    }
...
CMD:lockhouse(playerid, params[])
{
	foreach(new hid : Houses)
	{
		...
		{
			if(!Player_OwnsHouse(playerid, hid) && !Player_OwnsHouser(playerid, hid)) return Error(playerid, "You don't own this house.");
...
CMD:kasihkunci(playerid, params[])
{
	new hid, otherid;
	if(sscanf(params, "ud", otherid, hid)) return Usage(playerid, "/kasihkunci [playerid/name] [id] | /myhouse - for show info");
	...
	GetPlayerName(otherid, hData[hid][hOwnern], MAX_PLAYER_NAME);
	hData[hid][hVisit] = gettime();

	House_Refresh(hid);
	House_Save(hid);
	Info(playerid, "Anda memberikan kunci rumah id: %d kepada %s", hid, ReturnName(otherid));
	Info(otherid, "%s memberikan kunci rumah id: %d kepada anda", hid, ReturnName(playerid));
	return 1;
}
...
CMD:givehouse(playerid, params[])
{
	...
	{
		#if LIMIT_PER_PLAYER > 0
		if(Player_HouseCount(otherid) + 1 > 6) return Error(playerid, "Target player cant own any more houses.");
		#endif
	}
	else if(pData[otherid][pVip] == 6)
	{
		#if LIMIT_PER_PLAYER > 0
		if(Player_HouseCount(otherid) + 1 > 7) return Error(playerid, "Target player cant own any more houses.");
		#endif
	}
	else
	{
