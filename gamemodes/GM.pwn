...
stock const Float:arrAdminJail[][4] = {
    { -10.39, 2336.91, 24.30, 81.25 },
    { -10.86, 2329.84, 24.30, 83.10 },
    { -27.27, 2320.94, 24.30, 4.25 },
    { -18.47, 2322.33, 24.30, 348.72 }
};
//flash
...
enum
{
	...
	FAMILY_CLIPB,
	FAMILY_CLIPC,
	FAMILY_WITHDRAWCLIPA,
	FAMILY_WITHDRAWCLIPB,
	FAMILY_WITHDRAWCLIPC,
	FAMILY_DEPOSITCLIPA,
	FAMILY_DEPOSITCLIPB,
	FAMILY_DEPOSITCLIPC,
...
IsSANACar(carid)
{
	for(new v = 0; v < sizeof(SANAVeh); v++)
	{
	    if(carid == SANAVeh[v]) return 1;
	}
	return 0;
}
...
public OnGameModeInit()
{
    ...
    CreateDynamicPickup(1239, 23, 1552.9539,-1609.6826,13.3828, -1);
	...
	*/
	//new bank
	CreateDynamicPickup(1239, 23, 1027.6293,-1442.8956,14.4960, -1, -1, -1, 50);
	format(strings, sizeof(strings), "[BANK]\n{FFFFFF}/newrek - create new rekening");
	CreateDynamic3DTextLabel(strings, COLOR_LBLUE, 1027.6293,-1442.8956,14.4960, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // bank

	CreateDynamicPickup(1239, 23, 1021.3434,-1442.8960,14.5160, -1, -1, -1, 50);
	format(strings, sizeof(strings), "[BANK]\n{FFFFFF}/bank - access rekening");
	CreateDynamic3DTextLabel(strings, COLOR_LBLUE, 1021.3434,-1442.8960,14.5160, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // bank
	...
	CreateDynamicPickup(1241, 23, 1341.596801, 1569.870727, 3010.906250, -1, -1, -1, 50);
	format(strings, sizeof(strings), "[MYRICOUS PRODUCTION]\n{FFFFFF}/mix");
	CreateDynamic3DTextLabel(strings, COLOR_ORANGE2, 1341.596801, 1569.870727, 3010.906250, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1); // racik obat
...
public OnPlayerCommandPerformed(playerid, cmdtext[], success)
{
    if (!success)
    {
        Error(playerid, "command '/%s' does not exist use '/help' for list command.", cmdtext);
        return 0;
    }
    return 1;
}
...
{	
	...
	TextDrawShowForPlayer(playerid, NAMASERVERTD[0]);
	TextDrawShowForPlayer(playerid, NAMASERVERTD[1]);
	TextDrawShowForPlayer(playerid, NAMASERVERTD[2]);
	TextDrawShowForPlayer(playerid, NAMASERVERTD[3]);
	TextDrawShowForPlayer(playerid, NAMASERVERTD[4]);
	TextDrawShowForPlayer(playerid, NAMASERVERTD[5]);
	...
	if(IsPlayerConnected(playerid))
	{
		...
		else
		{
			...
			if(pData[playerid][pHBEMode] == 1) //simple
			{
				...
				/*PlayerTextDrawShow(playerid, TumbasTD[playerid][0]);
				PlayerTextDrawShow(playerid, TumbasTD[playerid][1]);
				PlayerTextDrawShow(playerid, TumbasTD[playerid][2]);
				PlayerTextDrawShow(playerid, TumbasTD[playerid][3]);
				PlayerTextDrawShow(playerid, TumbasTD[playerid][4]);
				PlayerTextDrawShow(playerid, TumbasTD[playerid][5]);
				PlayerTextDrawShow(playerid, TumbasTD[playerid][6]);
				PlayerTextDrawShow(playerid, TumbasTD[playerid][7]);
				PlayerTextDrawShow(playerid, TumbasTD[playerid][8]);
				PlayerTextDrawShow(playerid, TumbasTD[playerid][9]);
				PlayerTextDrawShow(playerid, TumbasTD[playerid][10]);
				PlayerTextDrawShow(playerid, TumbasTD[playerid][11]);*/
				PlayerTextDrawShow(playerid, TumbasTD[playerid][25]);
				PlayerTextDrawShow(playerid, TumbasTD[playerid][26]);
				PlayerTextDrawShow(playerid, TumbasTD[playerid][27]);
				PlayerTextDrawShow(playerid, TumbasTD[playerid][28]);
				PlayerTextDrawShow(playerid, TumbasTD[playerid][29]);
				// New Hun 2
                PlayerTextDrawShow(playerid, HopixelPTD[playerid][0]);
				PlayerTextDrawShow(playerid, HopixelPTD[playerid][1]);
				PlayerTextDrawShow(playerid, HopixelPTD[playerid][2]);
				PlayerTextDrawShow(playerid, HopixelPTD[playerid][3]);
				PlayerTextDrawShow(playerid, HopixelPTD[playerid][4]);
				PlayerTextDrawShow(playerid, HopixelPTD[playerid][5]);
				PlayerTextDrawShow(playerid, HopixelPTD[playerid][6]);
				PlayerTextDrawShow(playerid, HopixelPTD[playerid][7]);
				PlayerTextDrawShow(playerid, HopixelPTD[playerid][8]);
				PlayerTextDrawShow(playerid, HopixelPTD[playerid][9]);
				PlayerTextDrawShow(playerid, HopixelPTD[playerid][10]);
				PlayerTextDrawShow(playerid, HopixelPTD[playerid][16]);
				PlayerTextDrawShow(playerid, HopixelPTD[playerid][17]);
				PlayerTextDrawShow(playerid, HopixelPTD[playerid][18]);
				PlayerTextDrawShow(playerid, HopixelPTD[playerid][19]);
				PlayerTextDrawShow(playerid, HopixelPTD[playerid][20]);
...
/*	if(checkpointid == ShowRoomCP)
	...
	{
	 	Info(playerid, "KLIK TOMBOL "LG_E"'N' UNTUK MULAI BEKERJA (SUPIR BUS)");
	 	return 1;
	}
		...
		case CHECKPOINT_BAGGAGE:
		{
			if(pData[playerid][pBaggage] > 0)
			{
				...
				else if(pData[playerid][pBaggage] == 22)
				{
					...
					AddPlayerSalary(playerid, "Job(Baggage)", 300);
					Info(playerid, "Job(Baggage) telah masuk ke pending salary anda!");
					RemovePlayerFromVehicle(playerid);
					SetVehicleToRespawn(vehicleid);
					SetValidVehicleHealth(vehicleid, 1000);
			...
			if(pData[playerid][pSideJob] == 2)
			{
				...
				if(GetVehicleModel(vehicleid) == 431)
				{
					...
					else if(pData[playerid][pBus] == 35)
					{
						...
						pData[playerid][pCheckPoint] = CHECKPOINT_NONE;
						DisablePlayerRaceCheckpoint(playerid);
						AddPlayerSalary(playerid, "Sidejob(Bus)", 250);
						Info(playerid, "Sidejob(Bus) telah masuk ke pending salary anda!");
						RemovePlayerFromVehicle(playerid);
						SetVehicleToRespawn(vehicleid);
						SetValidVehicleHealth(vehicleid, 1000);
			...
			if(pData[playerid][pSideJob] == 1)
			{
				...
				if(GetVehicleModel(vehicleid) == 574)
				{
					...
					else if(pData[playerid][pSweeper] == 12)
					{
						...
						pData[playerid][pCheckPoint] = CHECKPOINT_NONE;
						DisablePlayerRaceCheckpoint(playerid);
						AddPlayerSalary(playerid, "Sidejob(Sweeper)", 400);
					...
					else if(pData[playerid][pSweeper] == 20)
					{
						...
						pData[playerid][pCheckPoint] = CHECKPOINT_NONE;
						DisablePlayerRaceCheckpoint(playerid);
						AddPlayerSalary(playerid, "Sidejob(Sweeper)", 450);
						Info(playerid, "Sidejob(Sweeper) telah masuk ke pending salary anda!");
						RemovePlayerFromVehicle(playerid);
						SetVehicleToRespawn(vehicleid);
						SetValidVehicleHealth(vehicleid, 1000);
			...
			if(pData[playerid][pSideJob] == 3)
			{
				...
				if(GetVehicleModel(vehicleid) == 530)
				{
					if(pData[playerid][pForklifter] == 1)
					{
						...
						pData[playerid][pForklifterLoad] = SetTimerEx("ForklifterLoadBox", 3000, false, "i", playerid);
						ShowProgressbar(playerid, "Load", 3);
					}
					else if(pData[playerid][pForklifter] == 2)
					{
						...
						pData[playerid][pForklifterUnLoad] = SetTimerEx("ForklifterUnLoadBox", 3000, false, "i", playerid);
						ShowProgressbar(playerid, "Unload", 3);
					}
					...
					else if(pData[playerid][pForklifter] == 11)
					{
						...
						DisablePlayerRaceCheckpoint(playerid);
						AddPlayerSalary(playerid, "Sidejob(Forklift)", 250);
			...
			if(pData[playerid][pSideJob] == 4)
			{
				...
				if(GetVehicleModel(vehicleid) == 572)
				{
					...
					else if(pData[playerid][pMower] == 12)
					{
						...
						pData[playerid][pCheckPoint] = CHECKPOINT_NONE;
						DisablePlayerRaceCheckpoint(playerid);
						AddPlayerSalary(playerid, "Sidejob(Mower)", 320);
...
public OnPlayerEnterCheckpoint(playerid)
{
	...
	/*if(pData[playerid][pCarSteal] == 1)
	{
	    if(AtCarsRobs[playerid] == 1)
	    {
			...
			switch (random(4))
			{
				  case 0: pData[playerid][pRedMoney] += 1500, Info(playerid, "Uang Kamu Bertambah $1500!");
				  case 1: pData[playerid][pRedMoney] += 2000, Info(playerid, "Uang Kamu Bertambah $2000!");
				  case 2: pData[playerid][pRedMoney] += 2500, Info(playerid, "Uang Kamu Bertambah $2500!");
				  case 3: pData[playerid][pRedMoney] += 3000, Info(playerid, "Uang Kamu Bertambah $3000!");
			}
	...
	if(pData[playerid][pSideJob] == 1)
	{
		...
		if(GetVehicleModel(vehicleid) == 574)
		{
			...
			if(IsPlayerInRangeOfPoint(playerid, 7.0,sweperpoint12))
			{
				...
				pData[playerid][pSweeperTime] = 120;
				DisablePlayerCheckpoint(playerid);
				AddPlayerSalary(playerid, "Sidejob(Sweeper)", 280);
				Info(playerid, "Sidejob(Sweeper) telah masuk ke pending salary anda!");
				RemovePlayerFromVehicle(playerid);
				SetVehicleToRespawn(vehicleid);
				SetValidVehicleHealth(vehicleid, 1000);
	...
	if(pData[playerid][pSideJob] == 2)
	{
		...
		if(GetVehicleModel(vehicleid) == 431)
		{
			...
			if(IsPlayerInRangeOfPoint(playerid, 7.0,buspoint27))
			{
				...
				pData[playerid][pBusTime] = 360;
				DisablePlayerCheckpoint(playerid);
				AddPlayerSalary(playerid, "Sidejob(Bus)", 290);
				Info(playerid, "Sidejob(Bus) telah masuk ke pending salary anda!");
				RemovePlayerFromVehicle(playerid);
				SetVehicleToRespawn(vehicleid);
				SetValidVehicleHealth(vehicleid, 1000);
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys){
	...
	if((newkeys & KEY_SECONDARY_ATTACK))
    {
        foreach(new did : Doors)
		{
			if(IsPlayerInRangeOfPoint(playerid, 2.8, dData[did][dExtposX], dData[did][dExtposY], dData[did][dExtposZ]))
			{
                ...
                if(pData[playerid][pMoney] < dData[did][dDuit]) return Error(playerid, "Uang mu tidak mencukupi untuk masuk/keluar!.");
				{
	...
	if(newkeys & KEY_FIRE && TaserData[playerid][TaserEnabled] && GetPlayerWeapon(playerid) == 0 && !IsPlayerInAnyVehicle(playerid) && TaserData[playerid][TaserCharged])
	{
		...
		pData[playerid][pActivityTime] = 0;
	    TaserData[playerid][ChargeTimer] = SetTimerEx("ChargeUp", 10000, false, "i", playerid);
		ShowProgressbar(playerid, "Charge", 10);
	    ...
	    for(new i, maxp = GetPlayerPoolSize(); i <= maxp; ++i)
		{
			...
			TaserData[i][GetupTimer] = SetTimerEx("TaserGetUp", 10000, false, "i", i);
			ShowProgressbar(playerid, "Getup", 10);
			break;
	    }
	}
	//-----[ Vehicle ]-----	
	if((newkeys & KEY_NO ))
	{
		...
		if(IsPlayerInRangeOfPoint(playerid, 1.0, 1704.6984, -1524.3541, 13.3736))
		{
	...
	if(PRESSED(KEY_FIRE) && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT && pData[playerid][pBreaking] && pData[playerid][pTargetPv] != -1)
	{
	    ...
	    if(IsValidVehicle(pvData[id][cVeh]))
	    {
			...
			if(pvData[id][cBreaken] != playerid)
			    return Error(playerid, "Kendaraan ini sudah rusak oleh seseorang!");
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	if(oldstate == PLAYER_STATE_DRIVER)
    {
		...
		//hud new 2
		PlayerTextDrawHide(playerid, HopixelPTD[playerid][21]);
		PlayerTextDrawHide(playerid, HopixelPTD[playerid][22]);
		PlayerTextDrawHide(playerid, HopixelPTD[playerid][23]);
		PlayerTextDrawHide(playerid, HopixelPTD[playerid][24]);
		PlayerTextDrawHide(playerid, HopixelPTD[playerid][25]);
		PlayerTextDrawHide(playerid, HopixelPTD[playerid][26]);
	...
	else if(newstate == PLAYER_STATE_DRIVER)
	{
		...
		{
			//hud new 2
			PlayerTextDrawShow(playerid, HopixelPTD[playerid][21]);
			PlayerTextDrawShow(playerid, HopixelPTD[playerid][22]);
			PlayerTextDrawShow(playerid, HopixelPTD[playerid][23]);
			PlayerTextDrawShow(playerid, HopixelPTD[playerid][24]);
			PlayerTextDrawShow(playerid, HopixelPTD[playerid][25]);
			PlayerTextDrawShow(playerid, HopixelPTD[playerid][26]);
		}
		else
		{
...
public OnPlayerWeaponShot(playerid, weaponid, hittype, hitid, Float:fX, Float:fY, Float:fZ)
{
	...
	if(1 <= weaponid <= 46)
	{
		...
		if(IsPlayerInRangeOfPoint(playerid, 200.0, 1176.557739, -1323.968994, 14.027276)) return Info(playerid, "Anda berada di zona hijau jangan rusuh!!!");
	}
	return 1;
}
...
function playerUpdate(playerid){
	new string[256], fac[24];
	format(string, sizeof(string), "%d", playerid);
	PlayerTextDrawSetString(playerid, TumbasTD[playerid][27], string);
	if(pData[playerid][pFaction] == 1){
		fac = "SAPD";
	...
	new za[300];
    format(za, sizeof za, "%s", fac);
    PlayerTextDrawSetString(playerid, TumbasTD[playerid][29], za);
...
function VehicleUpdate()
{
	for (new i = 1; i != MAX_VEHICLES; i ++) if(IsEngineVehicle(i) && GetEngineStatus(i))
    {
        if(GetVehicleFuel(i) > 0)
        {
            ...
            SetVehicleFuel(i, fuel - 5);

            if(GetVehicleFuel(i) >= 1 && GetVehicleFuel(i) <= 100)
            {
...
stock BarVehicle(playerid)
{
    ...
    if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
    {
         if(pData[playerid][pHBEMode] == 1)
         {
				...
				SetPlayerProgressBarValue(playerid, pData[playerid][HEALTHBAR], fDamage);

				SetPlayerProgressBarColour(playerid, pData[playerid][HEALTHBAR], ConvertDAMAGEColor(fDamage));
				SetPlayerProgressBarColour(playerid, pData[playerid][FUELBAR], ConvertFUELColor(fFuel));

				format(tstr, sizeof(tstr), "%s", GetVehicleName(vehicleid));
				PlayerTextDrawSetString(playerid, VEHICLENAME[playerid], tstr);

				format(tstr, sizeof(tstr), "%i Kmh", GetVehicleSpeedKMH(vehicleid));
				PlayerTextDrawSetString(playerid, SPEED[playerid], tstr);
         }
   }
}

function PlayerVehicleUpdate(playerid)
{
	new vehicleid = GetPlayerVehicleID(playerid);
	if(IsValidVehicle(vehicleid))
	{
		...
		if(GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		{
			...
			{
			    //PlayerTextDrawShow(playerid, TumbasTD[playerid][23]);
		...
		if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		{
			if(pData[playerid][pHBEMode] == 1)
			{
				...
				format(str, sizeof(str), "%d Fuel", nFuel);
                PlayerTextDrawSetString(playerid, HopixelPTD[playerid][22], str);

                format(str, sizeof(str), "%.0f KM/H", GetVehicleSpeed(vehicleid));
                PlayerTextDrawSetString(playerid, HopixelPTD[playerid][21], str);

                format(str, sizeof(str), "%s", GetLocation(x, y, z));
                PlayerTextDrawSetString(playerid, HopixelPTD[playerid][25], str);
			}
			else
			{
	...
	if(IsPlayerInAnyVehicle(playerid))
	{
		foreach(new i : Player)
		{
			if(IsPlayerInAnyVehicle(i) && GetPlayerVehicleID(i) == GetPlayerVehicleID(playerid))
			{
                ...
                SetPlayerCheckpoint(i, fX, fY, fZ, 1);
			}
		}
	}
...
{
	//IPHONE
	if(clickedid == IphoneTD[8])
	{
	    Info(playerid, "1");
	}
	if(clickedid == IphoneTD[9])
	{
	    Info(playerid, "2");
	}
	if(clickedid == IphoneTD[10])
	{
	    Info(playerid, "3");
	}
	if(clickedid == IphoneTD[12])
	{
	    Info(playerid, "4");
	}
	if(clickedid == IphoneTD[13])
	{
	    Info(playerid, "5");
	}
	if(clickedid == IphoneTD[22])
	{
	    Info(playerid, "6");
	}
	if(clickedid == IphoneTD[23])
	{
	    Info(playerid, "7");
	}
	if(clickedid == IphoneTD[24])
	{
	    Info(playerid, "8");
	}
	if(clickedid == IphoneTD[25])
	{
	    Info(playerid, "9");
	}
	
	/*if(clickedid == BankTD[41])
	{
	    DisplayPaycheck(playerid);
	}
	if(clickedid == BankTD[35])
	{
		...
		Info(playerid, "Bank withdraw "LG_E"$500");
	}
	if(clickedid == BankTD[36])
	{
		...
		Info(playerid, "Bank withdraw "LG_E"$1000");
	}
	if(clickedid == BankTD[37])
	{
	    if(pData[playerid][pBankMoney] < 2000)
			return Error(playerid, "Not enough balance!");

		GivePlayerMoneyEx(playerid, 2000);
		pData[playerid][pBankMoney] -= 2000;
		Info(playerid, "Bank withdraw "LG_E"$2000");
	}
	if(clickedid == BankTD[40])
	{
		...
		GivePlayerMoneyEx(playerid, -500);
		pData[playerid][pBankMoney] += 500;
		Info(playerid, "Bank deposit "LG_E"$500");
	}
	if(clickedid == BankTD[39])
	{
		...
		GivePlayerMoneyEx(playerid, -1000);
		pData[playerid][pBankMoney] += 1000;
		Info(playerid, "Bank deposit "LG_E"$1000");
	}
	if(clickedid == BankTD[38])
	{
		...
		GivePlayerMoneyEx(playerid, -2000);
		pData[playerid][pBankMoney] += 2000;
		Info(playerid, "Bank deposit "LG_E"$2000");
	}
	if(clickedid == BankTD[43])
	{
	    for(new i = 0; i < 47; i++)
		{
			TextDrawHideForPlayer(playerid, BankTD[i]);
		}
		for(new u = 0; u < 2; u++)
		{
			PlayerTextDrawHide(playerid, BankNew[playerid][u]);
		}
		SelectTextDraw(playerid, COLOR_LBLUE);
	}
	if(clickedid == BankTD[29])
	{
	    ShowPlayerDialog(playerid, DIALOG_BANKREKENING, DIALOG_STYLE_INPUT, ""LB_E"Bank", "Masukan jumlah uang:", "Transfer", "Cancel");
	}
	if(clickedid == BankTD[31])
	{
	    new mstr[512];
		format(mstr, sizeof(mstr), "{F6F6F6}You have "LB_E"%s {F6F6F6}in bank account.\n\nType in the amount you want to deposit below:", FormatMoney(pData[playerid][pBankMoney]));
		ShowPlayerDialog(playerid, DIALOG_BANKDEPOSIT, DIALOG_STYLE_INPUT, ""LB_E"Bank", mstr, "Deposit", "Cancel");
	}
	if(clickedid == BankTD[30])
	{
	    new mstr[512];
		format(mstr, sizeof(mstr), "{F6F6F6}You have "LB_E"%s {F6F6F6}in your bank account.\n\nType in the amount you want to withdraw below:", FormatMoney(pData[playerid][pBankMoney]));
		ShowPlayerDialog(playerid, DIALOG_BANKWITHDRAW, DIALOG_STYLE_INPUT, ""LB_E"Bank", mstr, "Withdraw", "Cancel");
	...
	if(clickedid == apptd) 
	{
		...
		new string[512];
		format(string, sizeof(string),"Tweeter\nBinory\nIsi Kuota");
...
function ShowBox(playerid, model, text[], textt[])
{
    if(GetPVarInt(playerid, "BoxFivemShown") != -1)
    {
        for(new txd; txd < 6; txd++)
        {
            PlayerTextDrawHide(playerid, BoxFivem[playerid][txd]);
        }
        KillTimer(GetPVarInt(playerid, "BoxFivemShown"));
    }
    PlayerTextDrawSetString(playerid, BoxFivem[playerid][4], text);
    PlayerTextDrawSetString(playerid, BoxFivem[playerid][2], textt);
    PlayerTextDrawSetPreviewModel(playerid, BoxFivem[playerid][5], model);
    for(new txd; txd < 6; txd++)
    {
        PlayerTextDrawShow(playerid, BoxFivem[playerid][txd]);
    }
    SetPVarInt(playerid, "BoxFivemShown", SetTimerEx("HideBoxFivem", 5000, false, "i", playerid));
    return 1;
}

function HideBoxFivem(playerid)
{
    SetPVarInt(playerid, "BoxFivemShown", -1);
    for(new txd; txd < 6; txd++)
    {
        PlayerTextDrawHide(playerid, BoxFivem[playerid][txd]);
    }
...
function ShowBox1(playerid, model, text[], textt[])
{
    if(GetPVarInt(playerid, "BoxFivemShown1") != -1)
    {
        PlayerTextDrawHide(playerid, BoxFivem[playerid][6]);
        PlayerTextDrawHide(playerid, BoxFivem[playerid][7]);
        PlayerTextDrawHide(playerid, BoxFivem[playerid][8]);
        PlayerTextDrawHide(playerid, BoxFivem[playerid][9]);
        PlayerTextDrawHide(playerid, BoxFivem[playerid][10]);
        PlayerTextDrawHide(playerid, BoxFivem[playerid][11]);
        KillTimer(GetPVarInt(playerid, "BoxFivemShown1"));
    }
    PlayerTextDrawSetString(playerid, BoxFivem[playerid][10], text);
    PlayerTextDrawSetString(playerid, BoxFivem[playerid][8], textt);
    PlayerTextDrawSetPreviewModel(playerid, BoxFivem[playerid][11], model);
    PlayerTextDrawShow(playerid, BoxFivem[playerid][6]);
    PlayerTextDrawShow(playerid, BoxFivem[playerid][7]);
    PlayerTextDrawShow(playerid, BoxFivem[playerid][8]);
    PlayerTextDrawShow(playerid, BoxFivem[playerid][9]);
    PlayerTextDrawShow(playerid, BoxFivem[playerid][10]);
    PlayerTextDrawShow(playerid, BoxFivem[playerid][11]);
    SetPVarInt(playerid, "BoxFivemShown1", SetTimerEx("HideBoxFivem1", 5000, false, "i", playerid));
    return 1;
}

function HideBoxFivem1(playerid)
{
    SetPVarInt(playerid, "BoxFivemShown1", -1);
    PlayerTextDrawHide(playerid, BoxFivem[playerid][6]);
    PlayerTextDrawHide(playerid, BoxFivem[playerid][7]);
    PlayerTextDrawHide(playerid, BoxFivem[playerid][8]);
    PlayerTextDrawHide(playerid, BoxFivem[playerid][9]);
...
{
	TextDrawShowForPlayer(playerid, IphoneTD[0]);
	TextDrawShowForPlayer(playerid, IphoneTD[1]);
	TextDrawShowForPlayer(playerid, IphoneTD[2]);
	TextDrawShowForPlayer(playerid, IphoneTD[3]);
	TextDrawShowForPlayer(playerid, IphoneTD[4]);
	TextDrawShowForPlayer(playerid, IphoneTD[5]);
	TextDrawShowForPlayer(playerid, IphoneTD[6]);
	TextDrawShowForPlayer(playerid, IphoneTD[7]);
	TextDrawShowForPlayer(playerid, IphoneTD[8]);
	TextDrawShowForPlayer(playerid, IphoneTD[9]);
	TextDrawShowForPlayer(playerid, IphoneTD[10]);
	TextDrawShowForPlayer(playerid, IphoneTD[11]);
	TextDrawShowForPlayer(playerid, IphoneTD[12]);
	TextDrawShowForPlayer(playerid, IphoneTD[13]);
	TextDrawShowForPlayer(playerid, IphoneTD[14]);
	TextDrawShowForPlayer(playerid, IphoneTD[15]);
	TextDrawShowForPlayer(playerid, IphoneTD[16]);
	TextDrawShowForPlayer(playerid, IphoneTD[17]);
	TextDrawShowForPlayer(playerid, IphoneTD[18]);
	TextDrawShowForPlayer(playerid, IphoneTD[19]);
	TextDrawShowForPlayer(playerid, IphoneTD[20]);
	TextDrawShowForPlayer(playerid, IphoneTD[21]);
	TextDrawShowForPlayer(playerid, IphoneTD[22]);
	TextDrawShowForPlayer(playerid, IphoneTD[23]);
	TextDrawShowForPlayer(playerid, IphoneTD[24]);
	TextDrawShowForPlayer(playerid, IphoneTD[25]);
	TextDrawShowForPlayer(playerid, IphoneTD[26]);
	TextDrawShowForPlayer(playerid, IphoneTD[27]);
	TextDrawShowForPlayer(playerid, IphoneTD[28]);
	TextDrawShowForPlayer(playerid, IphoneTD[29]);
	TextDrawShowForPlayer(playerid, IphoneTD[30]);
	TextDrawShowForPlayer(playerid, IphoneTD[31]);
	TextDrawShowForPlayer(playerid, IphoneTD[32]);
	TextDrawShowForPlayer(playerid, IphoneTD[33]);
	TextDrawShowForPlayer(playerid, IphoneTD[34]);
	TextDrawShowForPlayer(playerid, IphoneTD[35]);
	TextDrawShowForPlayer(playerid, IphoneTD[36]);
	SelectTextDraw(playerid, COLOR_LBLUE);
	return 1;
}
