...
enum E_FARM
{
	...
	farmWheat,
	farmOrange,
    STREAMER_TAG_3D_TEXT_LABEL:farmText,
    STREAMER_TAG_PICKUP:farmPickup,
    Float:farmX,
    Float:farmY,
    Float:farmZ,
    farmStatus,
 	...
 	if(rows)
  	{
		...
		while(i < rows)
		{
	        ...
	        format(str, sizeof str,"[Field ID:%d]\n"YELLOW_E"Field Price: "LG_E"%s{ffffff}\n{ffffff}Type use "AQUA"'/buy' "WHITE_E"to purchased this Field", wid, FormatMoney(FarmData[wid][farmPrice]));

	        if(strcmp(FarmData[wid][farmOwner], "-", true))
	            format(str, sizeof str,"[Field ID:%d]\n{ffffff}Field Name: "AQUA"%s\n{ffffff}Field Owner: "YELLOW_E"%s\n{ffffff}STATUS: %s", wid, FarmData[wid][farmName], FarmData[wid][farmOwner], stats);

	        FarmData[wid][farmText] = CreateDynamic3DTextLabel(str, ARWIN, FarmData[wid][farmX], FarmData[wid][farmY], FarmData[wid][farmZ]+0.5, 8.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, -1, -1, -1, 8.0);
	        FarmData[wid][farmPickup] = CreateDynamicPickup(1239, 23, FarmData[wid][farmX], FarmData[wid][farmY], FarmData[wid][farmZ]+0.2, -1);
            Iter_Add(Farm, wid);
		    i++;
		}
