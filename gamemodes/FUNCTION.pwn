...
function OnPlayerDataLoaded(playerid, race_check)
{
	...
	{		
		...
		if(pData[playerid][pPassword] > 10)
		{
			...
			new lstring[512];
			format(lstring, sizeof lstring, "This Account Is "LG_E"Registered!\n"WHITE_E"UCP Name: "LB_E"%s\n{ffffff}Attemps: {FF0000}0/3{ffffff}\nPlease Input Your Password", pData[playerid][pUCP]);
			ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Login To HOPIXEL", lstring, "Login", "Quit");
		}
	}
	else
	{
            ...
            SendClientMessage(playerid, COLOR_LIGHTRED, "Your UCP not registered, Please register your UCP in discord server!");
			SetTimerEx("KickTimer", 3000, 0, "i", playerid);

	}
	new Query[256];
	mysql_format(g_SQL, Query, sizeof(Query), "SELECT * FROM `banip` WHERE `IP` = '%s' LIMIT 1", ReturnIP(playerid));
	mysql_tquery(g_SQL, Query, "CheaterKontolMemek", "d", playerid);
	return 1;
}
...
function CheckUCPBan(playerid)
{
	new count = cache_num_rows(), admin[32], reason[64], banned;
	if(count > 0)
	{
	    cache_get_value_name(0, "bannedby", admin);
	    cache_get_value_name(0, "banreason", reason);
	    cache_get_value_name_int(0, "banned", banned);
	    if(banned == 1)
	    {
			new str[256];
			format(str, sizeof(str),"{FFFFFF}Your UCP has been Banned from this server\n{FF0000}Your UCP: {FFFFFF}%s\n{FF0000}Reason: {FFFFFF}%s\n{FF0000}Banned By: {FFFFFF}%s\n{FFFFFF}For Unbanned please visit our forum at {FF0000}https://discord.gg/hrp", pData[playerid][pUCP], reason, admin);
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "{FFFFFF}Banned Alert - UCP Ban", str, "Close", "");
...
function CheaterKontolMemek(playerid)
{
	new count = cache_num_rows(), ip[32];
	if(count > 0)
	{
	    cache_get_value_name(0, "IP", ip);
	    if(!strcmp(ReturnIP(playerid), ip, true))
	    {
	        KickEx(playerid);
			new string[312];
			format(string, sizeof(string), "{FFFFFF}Your character IP has been banned from the server\n\n{FFFFFF}For unbanned appeals please visit {FFFF00}https://discord.gg/hrp");
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Banned Notification", string, "Oke", "");
			return 1;
		}
	}
	else
	{
		new zQuery[256];
		mysql_format(g_SQL, zQuery, sizeof(zQuery), "SELECT * FROM `playerucp` WHERE `ucp` = '%e' LIMIT 1", pData[playerid][pUCP]);
		mysql_tquery(g_SQL, zQuery, "CheckUCPBan", "d", playerid);
	}
	return 1;
}
...
function AssignPlayerData(playerid)
{
	...
	format(pData[playerid][pTname], MAX_PLAYER_NAME, "%s", twname);
	cache_get_value_name(0, "ip", ip);
	...
	cache_get_value_name_int(0, "playercs", pData[playerid][pCs]);
	cache_get_value_name_int(0, "spawnsapd", pData[playerid][pSpawnSapd]);
	cache_get_value_name_int(0, "spawnsamd", pData[playerid][pSpawnSamd]);
	cache_get_value_name_int(0, "spawnsana", pData[playerid][pSpawnSana]);
	cache_get_value_name_int(0, "spawnpg", pData[playerid][pSpawnPg]);
	...
	cache_get_value_name_int(0, "cubat", pData[playerid][pCubat]);
	...
	cache_get_value_name_int(0, "phonebook", pData[playerid][pPhoneBook]);
	cache_get_value_name_int(0, "binory", pData[playerid][pBinory]);
	cache_get_value_name_int(0, "tapp", pData[playerid][pTwitter]);
	cache_get_value_name_int(0, "twitter", pData[playerid][pTweet]);
	cache_get_value_name_int(0, "twitterstatus", pData[playerid][pTogTweet]);
	...
	//stock pedagang
	cache_get_value_name_int(0, "sayam", pData[playerid][pSayam]);
	cache_get_value_name_int(0, "sburger", pData[playerid][pSburger]);
	cache_get_value_name_int(0, "snasbung", pData[playerid][pSnasi]);
	cache_get_value_name_int(0, "sairmineral", pData[playerid][pSmineral]);
	//////////
	...
	SendClientMessage(playerid, COLOR_YELLOW, "[!] "WHITE_E"Welcome to HOPIXEL ROLEPLAY");
...
function ChangeTwitUserName(otherplayer, nname[])
{
	...
	{
		new query[512];
	    format(query, sizeof(query), "UPDATE players SET twittername='%e' WHERE reg_id=%d", nname, pData[otherplayer][pID]);
		mysql_tquery(g_SQL, query);
...
function a_ChangeTwitterName(otherplayer, playerid, twname[])
{
	...
	else
	{
		...
		mysql_tquery(g_SQL, query);
		format(pData[otherplayer][pTname], MAX_PLAYER_NAME, "%s", twname);
...
function LoadStats(playerid, PlayersName[])
{
	...
	else
	{
		new email[40], admin, helper, level, levelup, vip, viptime, coin, regdate[40], lastlogin[40], money, bmoney, brek,
			jam, menit, detik, gender, age[40], faction, family, warn, job, job2, int, world, ucp[30], reg_id, phonestatus, phone, twitter[40], boost, boostttime, cs;
		...
		cache_get_value_index_int(0, 30, boost);
		cache_get_value_index_int(0, 31, boostttime);
		cache_get_value_index_int(0, 32, cs);
        ...
        new boostt[2000];
        format(boostt, sizeof(boostt), (!pData[playerid][pBooster] ? ("{FF0000}None") : ("{00FF00}Active")));
		...
		format(gstr,sizeof(gstr),"%sEmail: [%s] | Char Story:[%s{FFFFFF}] | Warning:[{ff0000}%d/20{ffffff}] | Last Login: [%s]\n", gstr, email, !pData[playerid][pCs] ? ("{FF0000}None") : ("{00FF00}Active"), warn, lastlogin);
		...
		if(vip != 0)
		{
			format(gstr,sizeof(gstr),"%sInterior: [%d] | Virtual World: [%d] | Register Date: [%s] | VIP Level: [%s"WHITE_E"] | VIP Time: [%s] | Roleplay Boost: [%s"WHITE_E"] | Boost Time: [%s]", gstr, int, world, regdate, name1, ReturnTimelapse(gettime(), viptime), boostt, ReturnTimelapse(gettime(), boosttime) );
		}
		else
		{
			format(gstr,sizeof(gstr),"%sInterior: [%d] | Virtual World: [%d] | Register Date: [%s] | VIP Level: [%s"WHITE_E"] | VIP Time: [None] | Roleplay Boost: [%s"WHITE_E"] | Boost Time: [None]", gstr, int, world, regdate, name1, boostt);
		}
...
function CheckPlayerSpawn3Titik(playerid)
{
	...
	{
		SetPlayerPos(playerid, 1769.5007, -1893.7563, 13.5626);
		SetPlayerFacingAngle(playerid, 268.1539);
	...
	{
		SetPlayerPos(playerid, 817.1029, -1347.1366, 13.5275);
		SetPlayerFacingAngle(playerid, 46.2542);
		SetPlayerInterior(playerid, 0);
		SetPlayerVirtualWorld(playerid, 0);
		pData[playerid][pSpawnList] = 0;
		ClearAnimations(playerid);
...
function JailPlayer(playerid)
{
    ...
    SetPlayerInterior(playerid, 0);
    new eek = RandomEx(0, 50);
	SetPlayerVirtualWorld(playerid, eek);
