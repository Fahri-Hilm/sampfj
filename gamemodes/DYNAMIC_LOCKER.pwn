...
CMD:createlocker(playerid, params[])
{
	...
	new type;
	if(sscanf(params, "d", type)) return Usage(playerid, "/createlocker [type, 1.SAPD 2.SAGS 3.SAMD 4.SANA 5.PEDAGANG 6.VIP Locker]");
...
CMD:editlocker(playerid, params[])
{
    ...
    else if(!strcmp(type, "type", true))
    {
        ...
        if(sscanf(string, "d", tipe))
            return Usage(playerid, "/editlocker [id] [type] [type, 1.SAPD 2.SAGS 3.SAMD 4.SANA 5.PEDAGANG 6.VIP Locker]");

        if(tipe < 1 || tipe > 6)
            return Error(playerid, "You must specify at least 1 - 6.");
