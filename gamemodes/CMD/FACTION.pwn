...
CMD:gov(playerid, params[])
{
	...
	if(pData[playerid][pFactionRank] < 1)
		return Error(playerid, "Only faction level 1-6");
	...
	else if(pData[playerid][pFaction] == 5)
	{
		...
		SendClientMessageToAll(COLOR_ORANGE2, "|___________ Pedagang News Announcement ___________|");
		SendClientMessageToAll(COLOR_WHITE, lstr);
	}
	return 1;
}
...
CMD:checkcitymoney(playerid, params)
{
	...
	ShowPlayerDialog(playerid, DIALOG_SERVERMONEY, DIALOG_STYLE_MSGBOX, "Indolax City Money", lstr, "Manage", "Close");
	return 1;
}
...
CMD:mix(playerid, params[])
{
    ...
    if(IsPlayerInRangeOfPoint(playerid, 3.0, 1328.1687, 1561.7307, 3010.9063))
    {
    	...
    	pData[playerid][pProducting] = SetTimerEx("CreateObat", 5000, false, "id", playerid, 1);
    	ShowProgressbar(playerid, "Meracik", 5);
	}
	return 1;
}
...
CMD:treatment(playerid, params[])
{
	...
	Info(otherid, "Kamu sedang ditreatment oleh medis, Mohon tenang!");
   	//ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
	pData[playerid][pProductingStatus] = 1;
   	pData[playerid][pProducting] = SetTimerEx("TreatmentPlayer", 5000, false, "iid", playerid, otherid, 1);
   	ShowProgressbar(playerid, "Treatment", 3);
	return 1;
}
...
function TreatmentPlayer(playerid, otherid)
{
	if(!IsPlayerConnected(playerid)) return 0;
	{
		TogglePlayerControllable(playerid, 1);
