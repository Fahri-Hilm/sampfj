CMD:acmds(playerid)
{
	...
	strcat(line3, "\n"BLUE_E"IP:RP "WHITE_E"- Anti-Cheat is actived.\n\
	"PINK_E"NOTE: All admin commands log is saved in database! | Abuse Commands? Kick And Demote Premanent!.");
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"IP:RP: "YELLOW_E"Staff Commands", line3, "OK","");
	return true;
}
...
CMD:setcs(playerid, params[])
{
	...
	pData[otherid][pCs] = 1;
	SendStaffMessage(COLOR_RED, "Admin %s has actived character story player name %s", pData[playerid][pAdminname], pData[otherid][pName]);
	Servers(otherid, "Your character story has been actived. by %s", pData[playerid][pAdminname]);
	return 1;
}

CMD:deletecs(playerid, params[])
{
	if(pData[playerid][pAdmin] < 2)
		if(pData[playerid][pHelper] == 0)
			return PermissionError(playerid);
	...
	new otherid;
	if(sscanf(params, "u", otherid))
        return Usage(playerid, "/deletecs [playerid/PartOfName]");
	if(!IsPlayerConnected(otherid))
		return Error(playerid, "Player not connected!");
	pData[otherid][pCs] = 0;
	SendStaffMessage(COLOR_RED, "Admin %s has non-actived character story player name %s", pData[playerid][pAdminname], pData[otherid][pName]);
	Servers(otherid, "Your character story has non-actived. by %s", pData[playerid][pAdminname]);
	return 1;
}
...
CMD:setsf(playerid, params[])
{
	...
	if(sscanf(params, "u", otherid))
        return Usage(playerid, "/setsf [playerid/PartOfName]");
	if(!IsPlayerConnected(otherid))
		return Error(playerid, "Player not connected!");
	pData[otherid][pSpawnSapd] = 0;
	pData[otherid][pSpawnSamd] = 0;
	pData[otherid][pSpawnPg] = 0;
	SendStaffMessage(COLOR_RED, "Atmint %s has unactive faction spawn player name %s", pData[playerid][pAdminname], pData[otherid][pName]);
	Servers(otherid, "Your spawn faction has been unctived. by %s", pData[playerid][pAdminname]);
	return 1;
}
...
CMD:freeze(playerid, params[])
{
    ...
    Servers(playerid, "You have frozen %s's movements.", pData[otherid][pName]);
...
CMD:unfreeze(playerid, params[])
{
    ...
    Servers(playerid, "You have unfrozen %s's movements.", pData[otherid][pName]);
...
CMD:jail(playerid, params[])
{
	...
	if(timeSec < 1 || timeSec > 1000)
	{
	    if(pData[playerid][pAdmin] < 5)
	 	{
			Error(playerid, "Jail time must remain between 1 and 1000 minutes");
	    	return 1;
	  	}
	}
...
CMD:id(playerid, params[])
{
	...
	Servers(playerid, "Name: %s(ID: %d) (%s) - Ping: (%d)", pData[otherid][pName], otherid, pData[otherid][pUCP], GetPlayerPing(otherid));
	return 1;
}
...
CMD:setskin(playerid, params[])
{
    ...
    Servers(playerid, "You have set %s's skin to ID: %d.", pData[otherid][pName], skinid);
    Servers(otherid, "%s has set your skin to ID: %d.", pData[playerid][pAdminname], skinid);
    return 1;
}
...
CMD:ans(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, ""YELLOW_E"Admin %s[%d] To %s[%d]:{FFFFFF} | %s", pData[playerid][pAdminname], playerid, pData[to_player][pName], to_player, message);
	return 1;
}

CMD:anc(playerid, params[])
{
    new text[225];
    ...
    if(sscanf(params,"s[225]",text))
        return Usage(playerid, "/anc [text]");

    SendClientMessageToAllEx(COLOR_RED,"Announcement %s %s: {FFFFFF}%s", GetStaffRank(playerid), pData[playerid][pAdminname], ColouredText(text));
    return 1;
}
...
CMD:obanucp(playerid, params[])
{
	if(pData[playerid][pAdmin] >= 1)
	{
		...
		if(sscanf(params, "s[64]s[64]", name, reason))
			return SendClientMessageEx(playerid, COLOR_WHITE, "USAGE: /obanucp [UCP] [Reason]");



		new characterQuery[178];
		mysql_format(g_SQL, characterQuery, sizeof(characterQuery), "SELECT * FROM `playerucp` WHERE `ucp` = '%s' LIMIT 1", name);
		mysql_tquery(g_SQL, characterQuery, "OnUCPBanned", "dss", playerid, name, reason);
...
CMD:unbanip(playerid, params[])
{
	...
	if(sscanf(params, "s[32]", IP))
	    return Error(playerid, "/unbanip [IP]");

	new string[128];
	format(string, sizeof(string), "DELETE FROM `banip` WHERE `IP` = '%s'", IP);
	mysql_tquery(g_SQL, string);

	SendClientMessageToAllEx(COLOR_LIGHTRED, "AdmCmd: IP %s has been unbanned from banned list", IP);
	return 1;
}
CMD:banip(playerid, params[])
{
	...
	if(sscanf(params, "s[32]s[32]", IP, reason))
		return SendSyntaxMessage(playerid, "/banip [ip] [reason]");

	new string[128];
	format(string, sizeof(string), "INSERT INTO `banip` (`IP`) VALUES('%s')", IP);
	mysql_tquery(g_SQL, string);

	SendClientMessageToAllEx(COLOR_LIGHTRED, "AdmCmd: IP %s has been banned from the server, reason: %s", IP, reason);
...
{
	new ucp[32], reason[64];

	if(pData[playerid][pAdmin] < 1)
	    return Error(playerid, "You don't have permission to use this command.");

	if (sscanf(params, "s[32]s[64]", ucp, reason))
	    return SendSyntaxMessage(playerid, "/unbanucp [ucp name] [reason]");

	SendClientMessageToAllEx(COLOR_LIGHTRED, "AdmCmd: UCP %s has been unbanned by %s, reason: %s", ucp, pData[playerid][pUCP], reason);

	new query[256];
	format(query,sizeof(query),"UPDATE `playerucp` SET `banned` = 0 WHERE `UCP` = '%s'", ucp);
...
CMD:banucp(playerid, params[])
{
	...
	if (sscanf(params, "us[64]", userid, reason))
	    return SendSyntaxMessage(playerid, "/banucp [playerid/name] [reason]");

    if (userid == INVALID_PLAYER_ID)
	    return Error(playerid, "You have specified an invalid player.");

	SendClientMessageToAllEx(COLOR_LIGHTRED, "AdmCmd: UCP %s has been banned by %s, reason: %s", pData[userid][pUCP], pData[playerid][pUCP], reason);

	new query[256];
	format(query,sizeof(query),"UPDATE `playerucp` SET `banned` = 1, `banreason` = '%s', `bannedby` = '%s' WHERE `ucp` = '%s'", reason, pData[playerid][pUCP], pData[userid][pUCP]);
	mysql_query(g_SQL, query);

	new str[256];
	format(str, sizeof(str),"{FFFFFF}Your UCP has been Banned from this server\n{FF0000}Reason: {FFFFFF}%s\n{FF0000}Banned By: {FFFFFF}%s\n{FFFFFF}For Unbanned please visit our forum at {FF0000}https://discor.gg/indolaxrp", reason, pData[playerid][pUCP]);
	ShowPlayerDialog(userid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "{FFFFFF}Banned Alert - UCP Ban", str, "Close", "");

	KickEx(userid);
	return 1;
}

function OnUCPBanned(playerid, ucp[], reason[])
{
	...
	if(rows)
	{
		...
		SendClientMessageToAllEx(COLOR_LIGHTRED, "AdmCmd: %s has Offline banned UCP %s", pData[playerid][pUCP], ucp);
		SendClientMessageToAllEx(COLOR_LIGHTRED, "Reason: %s", reason);

		format(query,sizeof(query),"UPDATE `playerucp` SET `banned` = 1, `banreason` = '%s', `bannedby` = '%s' WHERE `ucp` = '%s'", reason, pData[playerid][pUCP], ucp);
		mysql_query(g_SQL, query);
	}
	else
	{
	    Error(playerid, "That UCP Is doesn't exists on the database!");
	}
	return 1;
}
...
CMD:setvip(playerid, params[])
{
	...
	{
	    Usage(playerid, "/setvip <ID/Name> <level 0 - 6> <time (in days) 0 for permanent>");
	    return true;
	}
	if(!IsPlayerConnected(otherid))
        return Error(playerid, "Player belum masuk!");
...
CMD:setfaction(playerid, params[])
{
    ...
    if(sscanf(params, "udd", otherid, fid, rank))
        return Usage(playerid, "/setfaction [playerid/PartOfName] [1.SAPD, 2.SAGS, 3.SAMD, 4.SANEW 5.PEDAGANG] [rank 1-6]");
...
CMD:setleader(playerid, params[])
{
    ...
    if(sscanf(params, "ud", otherid, fid))
        return Usage(playerid, "/setleader [playerid/PartOfName] [0.None, 1.SAPD, 2.SAGS, 3.SAMD, 4.SANEW, 5.PEDAGANG]");
...
CMD:takebmoney(playerid, params[])
{
	if(pData[playerid][pAdmin] < 4)
		return PermissionError(playerid);

	new bmoney, otherid;
	if(sscanf(params, "ud", otherid, bmoney))
	{
	    Usage(playerid, "/takebmoney <ID/Name> <amount>");
	    return true;
	}
	...
	if(!IsPlayerConnected(otherid))
 	...
 	if(bmoney > pData[otherid][pBankMoney])
		return Error(playerid, "Player doesn't have enough bmoney to deduct from!");

	pData[otherid][pBankMoney] -= bmoney;
	SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s(%i) has taken away BankMoney "RED_E"%d {ffff00}from %s", pData[playerid][pAdminname], bmoney, pData[otherid][pName]);
	new str[150];
	format(str,sizeof(str),"Admin: %s mengambil %d BankMoney dari %s !", GetRPName(playerid), bmoney, GetRPName(otherid));
	LogServer("Admin", str);
	return 1;
}
...
CMD:givebmoneyall(playerid, params[])
{
	...
	foreach(new pid : Player)
	{
		...
		Servers(pid, "Admin %s telah memberikan %s kepada semua player online", pData[playerid][pAdminname], FormatMoney(bmoney));
		new str[150];
		format(str,sizeof(str),"Admin: %s memberikan bank money kesemua player sejumlah %s", GetRPName(playerid), FormatMoney(bmoney));
		LogServer("Admin", str);
	}
...
CMD:agive(playerid, params[])
{
	...
	if(IsPlayerConnected(playerid)) 
	{
		...
		/*	if(pData[playerid][pBandage] < ammount)
			...
			Info(playerid, "Anda telah berhasil memberikan perban kepada %s sejumlah %d.", pData[otherid][pName], ammount);
        ...
        else if(strcmp(name,"redmoney",true) == 0)
		{
			...
			Info(playerid, "Anda telah berhasil memberikan redmoney kepada %s sejumlah %d.", pData[otherid][pName], ammount);
			Info(otherid, "Admin %s telah berhasil memberikan redmoney kepada anda sejumlah %d.", pData[playerid][pAdminname], ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"medicine",true) == 0)
		{
			...
			Info(playerid, "Anda telah berhasil memberikan medicine kepada %s sejumlah %d.", pData[otherid][pName], ammount);
		...
		else if(strcmp(name,"snack",true) == 0) 
		{
			...
			Info(playerid, "Anda telah berhasil memberikan snack kepada %s sejumlah %d.", pData[otherid][pName], ammount);
		...
		else if(strcmp(name,"sprunk",true) == 0) 
		{
			...
			Info(playerid, "Anda telah berhasil memberikan Sprunk kepada %s sejumlah %d.", pData[otherid][pName], ammount);
		...
		else if(strcmp(name,"material",true) == 0) 
		{
			...
			Info(playerid, "Anda telah berhasil memberikan Material kepada %s sejumlah %d.", pData[otherid][pName], ammount);
		...
		else if(strcmp(name,"component",true) == 0) 
		{
			...
			Info(playerid, "Anda telah berhasil memberikan Component kepada %s sejumlah %d.", pData[otherid][pName], ammount);
		...
		else if(strcmp(name,"marijuana",true) == 0) 
		{
			...
			Info(playerid, "Anda telah berhasil memberikan Marijuana kepada %s sejumlah %d.", pData[otherid][pName], ammount);
		...
		{	
			...
			Info(playerid, "Anda telah berhasil memberikan Obat kepada %s sejumlah %d.", pData[otherid][pName], ammount);
		...
		else if(strcmp(name,"gps",true) == 0) 
		{
			...
			Info(playerid, "Anda telah berhasil memberikan GPS kepada %s sejumlah %d.", pData[otherid][pName], ammount);
...
CMD:setstat(playerid, params[])
{
	...
	if(IsPlayerConnected(playerid)) 
	{
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Gold {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], pData[otherid][pName], ammount);
		...
		{			
			...
			pData[otherid][pMedicine] = ammount;
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Medicine {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], pData[otherid][pName], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Medkit {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], pData[otherid][pName], ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Medicine{ffffff} kamu menjadi %d.", pData[playerid][pAdminname], ammount);
		...
		{			
			...
			pData[otherid][pSnack] = ammount;
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Snack {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], pData[otherid][pName], ammount);
		...
		{			
			...
			pData[otherid][pSprunk] = ammount;
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Sprunk {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], pData[otherid][pName], ammount);
		...
		{			
			...
			pData[otherid][pBandage] = ammount;
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Bandage {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], pData[otherid][pName], ammount);
		...
		{			
			...
			pData[otherid][pMaterial] = ammount;
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Material {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], pData[otherid][pName], ammount);
		...
		{			
			...
			pData[otherid][pComponent] = ammount;
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Component {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], pData[otherid][pName], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Food {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], pData[otherid][pName], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Seed {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], pData[otherid][pName], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Potato {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], pData[otherid][pName], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Orange {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], pData[otherid][pName], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Wheat {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], pData[otherid][pName], ammount);
		...
		{			
			...
			pData[otherid][pMarijuana] = ammount;
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Marijuana {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], pData[otherid][pName], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Fish {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], pData[otherid][pName], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Worm {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pAdminname], pData[otherid][pName], ammount);
...
CMD:moderatorhelp(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "{FF0000}Moderator Commands | Indolax Roleplay", string, "Close","");
	return 1;
}
...
CMD:aclaimpv(playerid, params[])
{
	...
	foreach(new i : PVehicles)
	{
		if(pvData[i][cClaim] > 0 && pvData[i][cClaimTime] > 0)
		{
			if(pvData[i][cOwner] == pData[otherid][pID])
			{
				...
				found++;
				Info(playerid, "Kamu telah mengclaim kendaraan player %s dengan model %s", pData[otherid][pName], GetVehicleModelName(pvData[i][cModel]));
	...
	{
		Info(playerid, "Kamu berhasil mengclaim %d kendaraan %s!", found, pData[otherid][pName]);
...
CMD:setphone(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "Admin %s telah menset nomor phonsel milik player %s", pData[playerid][pAdminname], pData[otherid][pName]);
...
CMD:setrek(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "Admin %s telah menset nomor rekening player %s", pData[playerid][pAdminname], pData[otherid][pName]);
...
CMD:getcord(playerid)
{
	...
	format(cord, sizeof(cord), "Kordinat:\n X: %f Y: %f Z: %f A: %0.2f", Zx, Zy, Zz, Za);
...
CMD:sethealth(playerid, params[])
{
	...
	if(sscanf(params, "ud", otherid))
        return Usage(playerid, "/sethealth [playerid id/name]");

	if(!IsPlayerConnected(otherid))
        return Error(playerid, "Player not connected!");

	SetPlayerHealthEx(otherid, 100);
	SetPlayerArmourEx(otherid, 100);
...
CMD:setjobtime(playerid, params[])
{
	...
	if(sscanf(params, "u", otherid))
	{
	    Usage(playerid, "/setjobtime <ID/Name>");
	    return true;
	}
	...
	pData[otherid][pSideJobTime] = 0;
	pData[otherid][pJobTime] = 0;

	SendClientMessageToAllEx(COLOR_BAN, "AdmCmd: %s jobtime Player %s", pData[playerid][pAdminname], pData[otherid][pName]);
	return true;
}
...
CMD:savealldata(playerid, params[])
{
	...
	SendServerMessage(playerid, "You've successfully all player data.");
	return 1;
}
CMD:buglock(playerid, params[])
{
	if(pData[playerid][pAdmin] < 1)
	    return PermissionError(playerid);
...
CMD:kepo(playerid, params[])
{
	...
	//SendStaffMessage(COLOR_RED, " player %s telah terbang kelangit ketujuh", pData[otherid][pName]);
