...
CMD:fill(playerid, params[])
{
	...
	foreach(new gsid : GStation)
	{
		if(IsPlayerInRangeOfPoint(playerid, 4.0, gsData[gsid][gsPosX], gsData[gsid][gsPosY], gsData[gsid][gsPosZ]))
		{
			...
			pData[playerid][pFillTime] = SetTimerEx("Filling", 5000, false, "i", playerid);
			ShowProgressbar(playerid, "Refuel", 5);
		}
	}
	return 1;
}
...
function Filling(playerid)
{
	foreach(new gsid : GStation)
	{
		if(!IsPlayerInRangeOfPoint(playerid, 4.0, gsData[gsid][gsPosX], gsData[gsid][gsPosY], gsData[gsid][gsPosZ]) && !IsPlayerInAnyVehicle(playerid) || GetVehicleFuel(GetPlayerVehicleID(playerid)) >= 999.0 || pData[playerid][pMoney] < GStationPrice)
		{
		...
		else
		{
			...
			pData[playerid][pFillPrice] = GStationPrice*5;
			gsData[pData[playerid][pFill]][gsStock] -= 1;
