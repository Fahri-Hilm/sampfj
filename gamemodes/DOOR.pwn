...
{
	new dquery[2048];
	mysql_format(g_SQL, dquery, sizeof(dquery), "UPDATE doors SET name='%s', password='%s', icon='%d', locked='%d', money='%d', admin='%d', vip='%d', faction='%d', family='%d', garage='%d', custom='%d', extvw='%d', extint='%d', extposx='%f', extposy='%f', extposz='%f', extposa='%f', intvw='%d', intint='%d', intposx='%f', intposy='%f', intposz='%f', intposa='%f', mapicon='%d' WHERE ID='%d'",
	dData[id][dName], dData[id][dPass], dData[id][dIcon], dData[id][dLocked], dData[id][dDuit], dData[id][dAdmin], dData[id][dVip], dData[id][dFaction], dData[id][dFamily], dData[id][dGarage], dData[id][dCustom], dData[id][dExtvw], dData[id][dExtint], dData[id][dExtposX], dData[id][dExtposY], dData[id][dExtposZ], dData[id][dExtposA], dData[id][dIntvw], dData[id][dIntint],
 	...
 	if(rows)
  	{
		...
		for(new i; i < rows; i++)
		{
		    ...
		    cache_get_value_name_int(i, "money", dData[did][dDuit]);
...
CMD:editdoor(playerid, params[])
{
    ...
    if(sscanf(params, "ds[24]S()[128]", did, type, string))
    {
        ...
        SendClientMessage(playerid, COLOR_YELLOW, "[NAMES]:{FFFFFF} location, interior, password, name, locked, admin, vip, money, faction, family, custom, virtual, iconmap");
        return 1;
    }
    ...
    else if(!strcmp(type, "money", true))
    {
        ...
        if(sscanf(string, "d", duit))
            return Usage(playerid, "/editdoor [id] [money]");

        dData[did][dDuit] = duit;
        Doors_Save(did);
		Doors_Updatelabel(did);
        SendAdminMessage(COLOR_RED, "%s has adjusted the money of entrance ID: %d to %s", pData[playerid][pAdminname], did, FormatMoney(duit));
    }
	...
	else if(!strcmp(type, "iconmap", true))
	{
	    ...
	    {
	        return Usage(playerid, "/editdoor [id] [iconmap] [iconid (0-63)]");
		}
		if(!(0 <= iconid <= 63))
		{
