...
{
	new dquery[2048];
	mysql_format(g_SQL, dquery, sizeof(dquery), "UPDATE gates SET model='%d', password='%s', admin='%d', vip='%d', faction='%d', family='%d', umum='%d', speed='%f', cX='%f', cY='%f', cZ='%f', cRX='%f', cRY='%f', cRZ='%f', oX='%f', oY='%f', oZ='%f', oRX='%f', oRY='%f', oRZ='%f' WHERE ID='%d'",
	gData[id][gModel], gData[id][gPass], gData[id][gAdmin], gData[id][gVip], gData[id][gFaction], gData[id][gFamily], gData[id][gUmum], gData[id][gSpeed], gData[id][gCX], gData[id][gCY], gData[id][gCZ], gData[id][gCRX], gData[id][gCRY], gData[id][gCRZ], gData[id][gOX], gData[id][gOY], gData[id][gOZ], gData[id][gORX], gData[id][gORY], gData[id][gORZ], id);
 	...
 	if(rows)
  	{
		...
		for(new i; i < rows; i++)
		{
		    ...
		    cache_get_value_name_int(i, "umum", gData[id][gUmum]);
...
CMD:gedit(playerid, params[])
{
	...
	else if(!strcmp(type, "faction", true))
    {
        ...
        if(fid < 0 || fid > 5)
            return Error(playerid, "Invalid value. Use 0 - 5 for type.");
	...
	else if(!strcmp(type, "umum", true))
    {
		gData[id][gUmum] = 1;
	    Servers(playerid, "You have set gate ID %d's to Umun.", id);
	    Gate_Save(id);
	}
