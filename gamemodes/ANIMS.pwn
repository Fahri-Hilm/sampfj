...
CMD:anim(playerid, params[])
{
    ...
    if(Index < 1 || Index > 1812)
        return Servers(playerid, "/anim [1-1812] Note: Press H Atau Menggunakan CMD /astop Untuk Menghentikan Anim");
...
CMD:parah(playerid, params[])
{
	new otherid,
		choice[32];
	//if(GetPlayerState(playerid) != PLAYER_STATE_ONFOOT || pData[playerid][pInjured] == 1) return Error(playerid, "You can't do at this moment.");
	if(sscanf(params, "us[32]", otherid, choice))
	{
		Usage(playerid, "/parah [id player] [1-4]");
		return 1;
	}
	if(strcmp(choice, "1", true) == 0)
	{
		ApplyAnimation(otherid,"BLOWJOBZ","BJ_COUCH_START_P",4.1, 0, 1, 1, 1, 1, 1);
		SetPVarInt(otherid, "UsingAnim", 1);
	}
	if(strcmp(choice, "2", true) == 0)
	{
		ApplyAnimation(otherid,"BLOWJOBZ","BJ_COUCH_START_W",4.1, 0, 1, 1, 1, 1, 1);
		SetPVarInt(otherid, "UsingAnim", 1);
	}
	if(strcmp(choice, "3", true) == 0)
	{
		ApplyAnimation(otherid,"BLOWJOBZ","BJ_COUCH_LOOP_P",4.1, 0, 1, 1, 1, 1, 1);
		SetPVarInt(otherid, "UsingAnim", 1);
	}
	if(strcmp(choice, "4", true) == 0)
	{
		ApplyAnimation(otherid,"BLOWJOBZ","BJ_COUCH_LOOP_W",4.1, 0, 1, 1, 1, 1, 1);
		SetPVarInt(otherid, "UsingAnim", 1);
	}
	return 1;
}
CMD:joged(playerid,params[])
{
	new anim,
		otherid;
    if(pData[playerid][pAdmin] < 6)
        return PermissionError(playerid);
	//if(GetPlayerState(p) != PLAYER_STATE_ONFOOT || pData[otherid][pInjured] == 1) return Error(playerid, "You can't do at this moment.");
	if(sscanf(params, "ud", otherid, anim)) return Usage(playerid,"/joged [id player] [1-10]");
	switch(anim){
		case 1: ApplyAnimation(otherid,"DANCING","DAN_Down_A",4.0, 1, 0, 0, 0, 0, 1);
		case 2: ApplyAnimation(otherid,"DANCING","DAN_Left_A",4.0, 1, 0, 0, 0, 0, 1);
		case 3: ApplyAnimation(otherid,"DANCING","DAN_Loop_A",4.0, 1, 0, 0, 0, 0, 1);
		case 4: ApplyAnimation(otherid,"DANCING","DAN_Right_A",4.0, 1, 0, 0, 0, 0, 1);
		case 5: ApplyAnimation(otherid,"DANCING","DAN_Up_A",4.0, 1, 0, 0, 0, 0, 1);
		case 6: ApplyAnimation(otherid,"DANCING","dnce_M_a",4.0, 1, 0, 0, 0, 0, 1);
		case 7: ApplyAnimation(otherid,"DANCING","dnce_M_b",4.0, 1, 0, 0, 0, 0, 1);
		case 8: ApplyAnimation(otherid,"DANCING","dnce_M_c",4.0, 1, 0, 0, 0, 0, 1);
		case 9: ApplyAnimation(otherid,"DANCING","dnce_M_c",4.0, 1, 0, 0, 0, 0, 1);
		case 10: ApplyAnimation(otherid,"DANCING","dnce_M_d",4.0, 1, 0, 0, 0, 0, 1);
		default: return Usage(playerid,"/joged	[id player] [1-10]");
	}
	return 1;
}
