...
CMD:ask(playerid, params[])
{
    ...
    if((reportid = Ask_Add(playerid, params)) != -1)
    {
        ...
        SendStaffMessage(COLOR_RED, "{7fff00}[ASK: #%d] "WHITE_E"%s (ID: %d)", reportid, pData[playerid][pName], playerid);
        SendStaffMessage(COLOR_RED, "QUESTION: %s", params);
        pData[playerid][pAskTime] = gettime() + 30;
    }
...
CMD:aask(playerid, params[])
{
    if(pData[playerid][pAdmin] < 1)
   		if(pData[playerid][pHelper] == 0)
     		return Error(playerid, "```\nKamu tidak memiliki hak untuk menggunakan ini!```");

    new reportid, msg[32];
    if(sscanf(params,"ds[32]", reportid, msg))
        return Usage(playerid, "```\n/aask [ask id] [jawaban] (/asks for a list)```");

    if((reportid < 0 || reportid >= MAX_ASKS) || !AskData[reportid][askExists])
        return Error(playerid, "```\nID Ask tidak valid, Listitem dari 0 sampai %d.```", MAX_ASKS);

    SendStaffMessage(COLOR_RED, "```\n%s telah menjawab pertanyaan %s(%d).```", pData[playerid][pAdminname], pData[AskData[reportid][askPlayer]][pName], AskData[reportid][askPlayer]);
