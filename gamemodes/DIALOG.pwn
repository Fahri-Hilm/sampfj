...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == DIALOG_LOGIN)
    {
		...
		else
		{
			...
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Hopixel - Roleplay", "You have mistyped your password too often (3 times).", "Okay", "");
				KickEx(playerid);
			}
			else
			{
			    ...
			    format(str, sizeof(str), "Welcome Back to Hopixel Roleplay Server!\n\nUCP Name: %s\n"WHITE_E"STATUS: %s\n"RED_E"Wrong Password!\n"WHITE_E"Attempts: %d", GetName(playerid), pp, pData[playerid][LoginAttempts]);
				ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Hopixel - Roleplay", str, "Login", "Abort");
			}
		}
        return 1;
    }
		...
		if(response)
		{
			...
			switch (listitem) 
			{
				case 0: 
				{
					...
					ShowPlayerDialog(playerid, DIALOG_SPAWN_1, DIALOG_STYLE_LIST, "Select Your Location", "» Ls Station\n» Ls Market", "Select", "Cancel");
	...
	if(dialogid == DIALOG_SETTINGS)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 3:
				{
					return cmd_togpm(playerid);
				}
				case 4:
				{
					return cmd_toglog(playerid);
				}
				case 5:
				{
					return cmd_togads(playerid);
				}
				case 6:
				{
					return cmd_togwt(playerid);
				}
			}
		}
	}
	if(dialogid == DIALOG_HBEMODE)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				{
				    /*
					ShowPlayerProgressBar(playerid, pData[playerid][FOODPROGRESS]);
					ShowPlayerProgressBar(playerid, pData[playerid][DRINKPROGRESS]);
					...
					ShowPlayerProgressBar(playerid, pData[playerid][HEALTHBAR]);
					*/
				...
				case 1:
				{
					...
					HidePlayerProgressBar(playerid, pData[playerid][FOODPROGRESS]);
					HidePlayerProgressBar(playerid, pData[playerid][DRINKPROGRESS]);
					*/
			...
			else if(bData[bizid][bType] == 3)
			{
				switch(listitem)
				{
					...
					case 1:
					{
						...
						ShowPlayerDialog(playerid, DIALOG_TOYBUY, DIALOG_STYLE_LIST, ""RED_E"HOPIXEL RP: "WHITE_E"Player Toys", string, "Select", "Cancel");
					}
					case 2:
					{
						if(price == 0)
	...
	if(dialogid == HOUSE_STORAGE)
	{
		...
		new string[200];
		if(!Player_OwnsHouse(playerid, pData[playerid][pInHouse]) && !Player_OwnsHouser(playerid, pData[playerid][pInHouse]))
	...
	{
		new houseid = pData[playerid][pInHouse];
		if(!Player_OwnsHouse(playerid, houseid) && !Player_OwnsHouser(playerid, pData[playerid][pInHouse])) return Error(playerid, "You don't own this house.");
		if(response)
		{
			switch (listitem)
			{
	...
	{
		new houseid = pData[playerid][pInHouse];
		if(!Player_OwnsHouse(playerid, pData[playerid][pInHouse]) && !Player_OwnsHouser(playerid, pData[playerid][pInHouse])) return Error(playerid, "You don't own this house.");
		if(response)
		{
			switch (listitem)
			{
	...
	{
		new houseid = pData[playerid][pInHouse];
		if(!Player_OwnsHouse(playerid, houseid) && !Player_OwnsHouser(playerid, pData[playerid][pInHouse])) return Error(playerid, "Ini bukan rumah anda!.");
		if(response)
		{
			switch (listitem)
			{
	...
	{
		new houseid = pData[playerid][pInHouse];
		if(!Player_OwnsHouse(playerid, pData[playerid][pInHouse]) && !Player_OwnsHouser(playerid, pData[playerid][pInHouse])) return Error(playerid, "Ini bukan rumah anda!.");
		if(response)
		{
			switch (listitem)
			{
		...
		if(response) 
		{
			...
			foreach(new i : PVehicles)
			{
				if(carid == pvData[i][cVeh])
				{
					...
					format(pvData[i][cPlate], 32, "Hopixel-%d", rand);
		...
		if(response)
		{
			switch(listitem)
			{
				...
				case 2: // change bone
				{
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSISI, DIALOG_STYLE_LIST, ""RED_E"HOPIXEL RP: "WHITE_E"Player Toys", finstring, "Select", "Cancel");
				}
				case 3:
				{
	...
	if(dialogid == DIALOG_HELP)
    {
		...
		switch(listitem)
		{
			case 0:
			{
				...
				strcat(str, "{7fffd4}TWITTER: {7fff00}/togtw /tw /makan\n");
	...
	if(dialogid == DIALOG_GPS_PROPERTIES)
	{
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					return cmd_myhouse(playerid);
				}
				case 1:
				{
					return cmd_mybisnis(playerid);
				}
				case 2:
				{
					return cmd_myvending(playerid);
				}
				case 3:
				{
					return cmd_mypv(playerid, "");
				}
			}
		}
		else 
	...
	if(dialogid == DIALOG_GPS_GENERAL)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				{
					pData[playerid][pGpsActive] = 1;
					SetPlayerRaceCheckpoint(playerid, 1, 1046.6656,-1422.6849,13.5560, 0.0, 0.0, 0.0, 3);//bank
					Gps(playerid, "Active!");
				}
				case 1:
				{
	...
	if(dialogid == DIALOG_TRACKPARK)
	{
		if(response)
		{
			...
			pData[playerid][pGpsActive] = 1;
			SetPlayerRaceCheckpoint(playerid,1, GarageData[i][garagePos][0], GarageData[i][garagePos][1], GarageData[i][garagePos][2], 0.0, 0.0, 0.0, 3.5);
			Gps(playerid, "Custom Garage Checkpoint targeted! (%s)", GetLocation(GarageData[i][garagePos][0], GarageData[i][garagePos][1], GarageData[i][garagePos][2]));
		}
	}
	    ...
	    if(response)
	    {
	        switch(listitem)
			{
    			...
    			case 1:
    			{
			        ...
			        if(listitem == 0)
			        {
			            RespawnVehicle(pvData[id][cVeh]);
			            SendServerMessage(playerid, "Your {00FFFF}%s {FFFFFF}has been unstucked!", GetVehicleName(pvData[id][cVeh]));
					}
				}
			}
		}
	...
	if(dialogid == DIALOG_LOCKVEH)
	{
	    if(response)
	    {
	        new count;
         	foreach(new i : PVehicles)
	        {
	            if(pvData[i][cClaim] == 0 && pvData[i][cClaimTime] == 0 && pvData[i][cGaraged] == -1)
	            {
	    ...
	    if(response)
	    {
         	...
         	foreach(new i : PVehicles)
	        {
 				if(pvData[i][cImpounded] >= 1)
				{
	            	if(pvData[i][cOwner] == pData[playerid][pID] && count++ == listitem)
	            	{
	            	    if(pData[playerid][pMoney] < pvData[i][cImpoundPrice])
		...
		if(response)
		{
			switch (listitem)
			{
				...
				case 2:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_GUDANGPEDAGANG, DIALOG_STYLE_LIST, "PEDAGANG", string, "Pilih", "Batal");
				}
			}
		}
		return 1;
	}
	if(dialogid == MENU_PED)
	{
		if(response)
		{
			switch (listitem)
			{
				case 0:
				{
					if(pData[playerid][pSmineral] > 100) return Error(playerid, "Product full of stock!");
					pData[playerid][pActivity] = SetTimerEx("JusJembut", 5000, false, "d", playerid);
					ShowProgressbar(playerid, "Proses", 5);
					
				}
				case 1:
				{
					if(pData[playerid][pSayam] > 100) return Error(playerid, "Product full of stock!");
					pData[playerid][pActivity] = SetTimerEx("AyamTiren", 5000, false, "d", playerid);
					ShowProgressbar(playerid, "Proses", 5);
				}
				case 2:
				{
					if(pData[playerid][pSnasi] > 100) return Error(playerid, "Product full of stock!");
					pData[playerid][pActivity] = SetTimerEx("NasiBasi", 5000, false, "d", playerid);
					ShowProgressbar(playerid, "Proses", 5);
				}
				case 3:
				{
					if(pData[playerid][pSburger] > 100) return Error(playerid, "Product full of stock!");
					pData[playerid][pActivity] = SetTimerEx("BurgerPig", 5000, false, "d", playerid);
					ShowProgressbar(playerid, "Proses", 5);
				}
			}
		}
	...
	if(dialogid == DIALOG_GUDANGPEDAGANG)
	{
		if(response)
		{
			switch (listitem)
			{
				case 0:
				{
					if(pData[playerid][pSmineral] < 10) return Error(playerid, "Product out of stock!");
					if(pData[playerid][pMineral] >= 25) return Error(playerid, "Es Susu too full in your inventory! Maximal 25.");
					pData[playerid][pMineral] += 10;
					pData[playerid][pSmineral] -= 10;
				}
				case 1:
				{
					if(pData[playerid][pSayam] < 10) return Error(playerid, "Product out of stock!");
					if(pData[playerid][pAyam] >= 25) return Error(playerid, "Jus Jeruk too full in your inventory! Maximal 25.");
					pData[playerid][pAyam] += 10;
					GivePlayerMoneyEx(playerid, -2);
					pData[playerid][pSayam] -= 10;
				}
				case 2:
				{
					if(pData[playerid][pSnasi] < 10) return Error(playerid, "Product out of stock!");
					if(pData[playerid][pNasi] >= 25) return Error(playerid, "Nasi bungkus too full in your inventory! Maximal 25.");
					pData[playerid][pNasi] += 10;
					GivePlayerMoneyEx(playerid, -2);
					pData[playerid][pSnasi] -= 10;
				}
				case 3:
				{
					if(pData[playerid][pSburger] < 10) return Error(playerid, "Product out of stock!");
					if(pData[playerid][pBurger] >= 25) return Error(playerid, "Siomay too full in your inventory! Maximal 25.");
					pData[playerid][pBurger] += 10;
					GivePlayerMoneyEx(playerid, -2);
					pData[playerid][pSburger] -= 10;
				}
				case 4:
				{
	...
	if(dialogid == ST_MENU)
	{
		if(response)
		{
			switch(listitem)
			{
				...
				case 3:
				{
					ShowPlayerDialog(playerid, ST_BORAX, DIALOG_STYLE_LIST, "Gudang Borax", "Withdraw\nDeposit", "Select","Cancel");
				}
				case 4:
				{
		...
		if(response)
		{
			...
			switch(listitem)
			{
				case 0:
				{
					...
					format(str, sizeof str,"Borax: %d\n\nSilakan Masukkan jumlah untuk Mengambil", stData[stid][stBorax]);
				}
				case 1:
				{
					pData[playerid][pMenuType] = 2;
					format(str, sizeof str,"Borax: %d\n\nSilakan Masukan Jumlah untuk Meletakkan nya", stData[stid][stBorax]);
				}
			}
			ShowPlayerDialog(playerid, ST_BORAX2, DIALOG_STYLE_INPUT, "Borax Menu", str, "Input","Cancel");
		}
	}
	if(dialogid == ST_BORAX2)
	{
		if(response)
		{
			...
			if(pData[playerid][pMenuType] == 1)
			{
				...
				if(stData[stid][stBorax] < amount) return Error(playerid, "Tidak Cukup Borax");

				if((pData[playerid][pBorax] + amount) >= 501)
					return Error(playerid, "Anda telah mencapai maksimum Borax");

				pData[playerid][pBorax] += amount;
				stData[stid][stBorax] -= amount;
				Storage_Save(stid);
				Info(playerid, "Anda berhasil mengambil Borax sebanyak %d", amount);
			}
			else if(pData[playerid][pMenuType] == 2)
			{
				...
				if(pData[playerid][pBorax] < amount) return Error(playerid, "Tidak Cukup Borax");

				if((stData[stid][stBorax] + amount) >= MAX_STORAGE_INT)
					return Error(playerid, "Anda telah mencapai maksimum Borax");
	...
	if(dialogid == ST_REDMONEY2)
	{
		if(response)
		{
			...
			if(pData[playerid][pMenuType] == 1)
			{
				...
				if(stData[stid][stUangMerah] < amount) return Error(playerid, "Tidak Cukup Uang Merah");

				if((pData[playerid][pRedMoney] + amount) >= 501)
					return Error(playerid, "Anda telah mencapai maksimum Uang Merah");
			...
			else if(pData[playerid][pMenuType] == 2)
			{
				...
				if(pData[playerid][pRedMoney] < amount) return Error(playerid, "Tidak Cukup Uang Merah");
	...
	if(dialogid == DIALOG_WEAPONSAPD)
	{
		if(response)
		{
			switch (listitem) 
			{
				...
				case 4:
				{
					GivePlayerWeaponEx(playerid, 22, 20);
				...
				case 9:
				{
					if(pData[playerid][pFactionRank] < 3)
						return Error(playerid, "You are not allowed!");
					GivePlayerWeaponEx(playerid, 29, 20);
			...
			switch(listitem)
			{
				case 0:
				{
					...
					if(GetNearestVehicleToPlayer(playerid, 3.8, false) == pData[playerid][pMechVeh])
					{
						...
						pData[playerid][pMechanic] = SetTimerEx("EngineFix", 1000, false, "id", playerid, pData[playerid][pMechVeh]);
						ShowProgressbar(playerid, "Fixing", 5);
					}
					else
					{
				...
				case 1:
				{
					...
					if(GetNearestVehicleToPlayer(playerid, 3.8, false) == pData[playerid][pMechVeh])
					{
						...
						pData[playerid][pMechanicStatus] = 1;
						pData[playerid][pMechanic] = SetTimerEx("BodyFix", 1000, false, "id", playerid, pData[playerid][pMechVeh]);
						ShowProgressbar(playerid, "Fixing", 5);
					}
					else
					{
				...
				case 15:
				{
					if(IsAtMech(playerid))
					{
						...
						{	
							...
							pData[playerid][pMechanicStatus] = 1;
							pData[playerid][pMechanic] = SetTimerEx("ModifCar", 1000, false, "id", playerid, pData[playerid][pMechVeh]);
							ShowProgressbar(playerid, "Modif", 5);
						}
						else
						{
							KillTimer(pData[playerid][pMechanic]);
	...
	if(dialogid == DIALOG_SERVICE_COLOR2)
	{
		if(response)
		{
			...
			{	
				...
				pData[playerid][pMechanicStatus] = 1;
				pData[playerid][pMechanic] = SetTimerEx("SprayCar", 1000, false, "id", playerid, pData[playerid][pMechVeh]);
				ShowProgressbar(playerid, "Modif", 5);
			}
			else
			{
	...
	if(dialogid == DIALOG_SERVICE_PAINTJOB)
	{
		if(response)
		{
			...
			{	
				...
				pData[playerid][pMechanicStatus] = 1;
				pData[playerid][pMechanic] = SetTimerEx("PaintjobCar", 1000, false, "id", playerid, pData[playerid][pMechVeh]);
				ShowProgressbar(playerid, "Modif", 5);
			}
			else
			{
			...
			switch(listitem)
			{
				case 0:
				{
					...
					if(GetNearestVehicleToPlayer(playerid, 3.8, false) == pData[playerid][pMechVeh])
					{
						...
						pData[playerid][pMechanicStatus] = 1;
						pData[playerid][pMechanic] = SetTimerEx("NeonCar", 1000, false, "id", playerid, pData[playerid][pMechVeh]);
		...
		if(response)
		{
			switch(listitem)
			{
				case 0: //slc pistol
				{
					...
					pData[playerid][pArmsDealer] = SetTimerEx("CreateGun", 7000, false, "idd", playerid, WEAPON_SILENCED, 70);
					ShowProgressbar(playerid, "Create", 7);
				}
				case 1: //colt45 9mm
				{
					if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda masih memiliki activity progress!");
					...
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealerStatus] = 1;
					pData[playerid][pArmsDealer] = SetTimerEx("CreateGun", 7000, false, "idd", playerid, WEAPON_COLT45, 70);
					ShowProgressbar(playerid, "Create", 7);
				}
				case 2: //deagle
				{
					...
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealerStatus] = 1;
					pData[playerid][pArmsDealer] = SetTimerEx("CreateGun", 7000, false, "idd", playerid, WEAPON_DEAGLE, 70);
					ShowProgressbar(playerid, "Create", 7);
				}
				case 3: //shotgun
				{
					...
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealerStatus] = 1;
					pData[playerid][pArmsDealer] = SetTimerEx("CreateGun", 7000, false, "idd", playerid, WEAPON_SHOTGUN, 50);
					ShowProgressbar(playerid, "Create", 7);
				}
				case 4: //ak-47
				{
					...
					ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 0, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealerStatus] = 1;
					pData[playerid][pArmsDealer] = SetTimerEx("CreateGun", 7000, false, "idd", playerid, WEAPON_AK47, 100);
					ShowProgressbar(playerid, "Create", 7);
				}
			}
		}
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					...
					if(pData[playerid][pMoney] < MedicinePrice) return Error(playerid, "Not enough money.");
				...
				case 1:
				{
					...
					if(pData[playerid][pMoney] < MedkitPrice) return Error(playerid, "Not enough money.");
				...
				case 2:
				{
					if(Apotek < 10) return Error(playerid, "Product out of stock!");
					if(pData[playerid][pFaction] != 3) return Error(playerid, "You are not a medical member.");
					if(pData[playerid][pMoney] < 100) return Error(playerid, "Not enough money.");
	...
	if(dialogid == DIALOG_BANKDEPOSIT)
	{
		...
		else
		{
			...
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"HOPIXEL RP: "LB_E"Bank", lstr, "Close", "");
		}
	}
			...
			switch(listitem)
			{
				case 0:
				{
					...
					else
					{
						...
						busASU = CreateVehicle(431, 1704.6984, -1524.3541, 13.3736, 180.0043, 2, 2, 600000, 1);
                        PutPlayerInVehicle(playerid, busASU, 0);
	...
	if(dialogid == DIALOG_MY_WS)
	{
		if(response)
		{
			...
			pData[playerid][pLoc] = wid;
			SetPlayerRaceCheckpoint(playerid,1, FarmData[wid][farmX], FarmData[wid][farmY], FarmData[wid][farmZ], 0.0, 0.0, 0.0, 3.5);
			Info(playerid, "Ikuti checkpoint untuk menemukan Farm anda!");
			return 1;
		}
	}
	if(dialogid == WS_MENU)
	...
	if(dialogid == WS_COMPONENT2)
	{
		if(response)
		{
			...
			if(pData[playerid][pMenuType] == 1)
			{
				...
				if((pData[playerid][pComponent] + amount) >= 20000)
					return Error(playerid, "You've reached maximum of Component");
	...
	if(dialogid == DIALOG_VENDING_NAME)
	{
		...
		else return cmd_vendingmanage(playerid, "\0");
		return 1;
	}
	if(dialogid == DIALOG_VENDING_VAULT)
	{
		...
		if(response)
		{
			switch(listitem)
			{
				...
				{
					new mstr[512];
					format(mstr,sizeof(mstr),"Uang kamu: %s.\n\nMasukkan berapa banyak uang yang akan kamu simpan di dalam Vending ini", FormatMoney(pData[playerid][pMoney]));
	...
	if(dialogid == DIALOG_VENDING_EDITPROD)
	{
		...
		if(PlayerOwnVending(playerid, vid))
		{
			if(response)
			{
				...
				return cmd_vendingmanage(playerid, "\0");
		}
		return 1;
	}
	...
	if(dialogid == TRUNK_STORAGE)
	{
		...
		if(x == pvData[i][cVeh])
		{
			...
			if(response)
			{
				...
				{
					ShowPlayerDialog(playerid, TRUNK_BORAX, DIALOG_STYLE_LIST, "Borax Safe", "Withdraw from safe\nDeposit into safe", "Select", "Back");
				}
				else if(listitem == 5)
				{
					ShowPlayerDialog(playerid, TRUNK_OBOR, DIALOG_STYLE_LIST, "Olahan Borax Safe", "Withdraw from safe\nDeposit into safe", "Select", "Back");
				}
				else if(listitem == 6)
				{
					ShowPlayerDialog(playerid, TRUNK_SEED, DIALOG_STYLE_LIST, "Seed Safe", "Withdraw from safe\nDeposit into safe", "Select", "Back");
				}
				else if(listitem == 7)
				{
					ShowPlayerDialog(playerid, BARANG_BATU, DIALOG_STYLE_LIST, "BATU", "Withdraw from safe\nDeposit into safe", "Select", "Back");
				}
				else if(listitem == 8)
				{
					ShowPlayerDialog(playerid, BARANG_CUBAT, DIALOG_STYLE_LIST, "CUCIAN BATU", "Withdraw from safe\nDeposit into safe", "Select", "Back");
				}
				else if(listitem == 9)
				{
					ShowPlayerDialog(playerid, BARANG_BESI, DIALOG_STYLE_LIST, "BESI", "Withdraw from safe\nDeposit into safe", "Select", "Back");
				}
				else if(listitem == 10)
				{
					ShowPlayerDialog(playerid, BARANG_EMAS, DIALOG_STYLE_LIST, "EMAS", "Withdraw from safe\nDeposit into safe", "Select", "Back");
				}
				else if(listitem == 11)
				{
					ShowPlayerDialog(playerid, BARANG_BERLIAN, DIALOG_STYLE_LIST, "BERLIAN", "Withdraw from safe\nDeposit into safe", "Select", "Back");
				}
			}
		}
		return 1;
	}
	/*if(dialogid == BARANG_LEGAL)
	{
	   	new x = GetNearestVehicleToPlayer(playerid, 3.5, false);
	...
	if(dialogid == BARANG_BATU)
	{
		if(response)
		{
			switch (listitem)
			{
				...
				{
					new str[128];
					format(str, sizeof(str), "Please enter how much batu you wish to withdraw from the safe:");
					ShowPlayerDialog(playerid, BARANG_AMBILBATU, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				}
				case 1:
				{
					new str[128];
					format(str, sizeof(str), "Please enter how much batu you wish to deposit into the safe:");
					ShowPlayerDialog(playerid, BARANG_MASUKINBATU, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
				}
			}
		}
		return 1;
	}
	if(dialogid == BARANG_AMBILBATU)
	{
		if(response)
		{
			new x = GetNearestVehicleToPlayer(playerid, 3.5, false);
			...
			if(x == pvData[i][cVeh])
			{
				...
				{
					new str[128];
					format(str, sizeof(str), "Safe Balance: %d\n\nPlease enter how much batu you wish to withdraw from the safe:", vsData[carid][tBatu]);
					ShowPlayerDialog(playerid, BARANG_AMBILBATU, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
					return 1;
				}

				if(amount < 1 || amount > vsData[carid][tBatu])
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much batu you wish to withdraw from the safe:", vsData[carid][tBatu]);
					ShowPlayerDialog(playerid, BARANG_AMBILBATU, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				...
				else if(pData[playerid][pBatu] > 7)
				{
				     Error(playerid, "cuman bisa ambil 7 batu");
				     return 1;
				}
				vsData[carid][tBatu] -= amount;
				pData[playerid][pBatu] += amount;
	...
	if(dialogid == BARANG_MASUKINBATU)
	{
		if(response)
		{
			new x = GetNearestVehicleToPlayer(playerid, 3.5, false);
			...
			if(x == pvData[i][cVeh])
			{
				new carid = pvData[i][cVeh];
				new amount = strval(inputtext);
				new total = vsData[carid][tBatu] + amount;
				if(isnull(inputtext))
				{
					new str[128];
					format(str, sizeof(str), "Safe Balance: %d\n\nPlease enter how much batu you wish to deposit into the safe:", vsData[carid][tBatu]);
					ShowPlayerDialog(playerid, BARANG_MASUKINBATU, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
					return 1;
				}
				if(amount < 1 || amount > pData[playerid][pBatu] || total > 14)
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much batu you wish to deposit into the safe:", vsData[carid][tBatu]);
					ShowPlayerDialog(playerid, BARANG_MASUKINBATU, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
	...
	if(dialogid == BARANG_CUBAT)
	{
		if(response)
		{
			switch (listitem)
			{
				...
				{
					new str[128];
					format(str, sizeof(str), "Please enter how much CucianBatu you wish to withdraw from the safe:");
					ShowPlayerDialog(playerid, BARANG_AMBILCUBAT, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				}
				case 1:
				{
					new str[128];
					format(str, sizeof(str), "Please enter how much CucianBatu you wish to deposit into the safe:");
					ShowPlayerDialog(playerid, BARANG_MASUKINCUBAT, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
				}
			}
		}
		return 1;
	}
	if(dialogid == BARANG_AMBILCUBAT)
	{
		if(response)
		{
			new x = GetNearestVehicleToPlayer(playerid, 3.5, false);
			...
			if(x == pvData[i][cVeh])
			{
				...
				{
					new str[128];
					format(str, sizeof(str), "Safe Balance: %d\n\nPlease enter how much CucianBatu you wish to withdraw from the safe:", vsData[carid][tCubat]);
					ShowPlayerDialog(playerid, BARANG_AMBILCUBAT, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
					return 1;
				}

				if(amount < 1 || amount > vsData[carid][tCubat])
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much CucianBatu you wish to withdraw from the safe:", vsData[carid][tCubat]);
					ShowPlayerDialog(playerid, BARANG_AMBILCUBAT, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				...
				else if(pData[playerid][pCubat] > 7)
				{
				     Error(playerid, "cuman bisa ambil 7 CucianBatu");
				     return 1;
				}
				vsData[carid][tCubat] -= amount;
				pData[playerid][pCubat] += amount;
	...
	if(dialogid == BARANG_MASUKINCUBAT)
	{
		if(response)
		{
			new x = GetNearestVehicleToPlayer(playerid, 3.5, false);
			...
			if(x == pvData[i][cVeh])
			{
				new carid = pvData[i][cVeh];
				new amount = strval(inputtext);
				new total = vsData[carid][tCubat] + amount;
				if(isnull(inputtext))
				{
					new str[128];
					format(str, sizeof(str), "Safe Balance: %d\n\nPlease enter how much CucianBatu you wish to deposit into the safe:", vsData[carid][tCubat]);
					ShowPlayerDialog(playerid, BARANG_MASUKINCUBAT, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
					return 1;
				}
				if(amount < 1 || amount > pData[playerid][pCubat] || total > 21)
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much CucianBatu you wish to deposit into the safe:", vsData[carid][tCubat]);
					ShowPlayerDialog(playerid, BARANG_MASUKINCUBAT, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
	...
	if(dialogid == BARANG_BESI)
	{
		if(response)
		{
			switch (listitem)
			{
				...
				{
					new str[128];
					format(str, sizeof(str), "Please enter how much Besi you wish to withdraw from the safe:");
					ShowPlayerDialog(playerid, BARANG_AMBILBESI, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				}
				case 1:
				{
					new str[128];
					format(str, sizeof(str), "Please enter how much Besi you wish to deposit into the safe:");
					ShowPlayerDialog(playerid, BARANG_MASUKINBESI, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
				}
			}
		}
		return 1;
	}
	if(dialogid == BARANG_AMBILBESI)
	{
		if(response)
		{
			new x = GetNearestVehicleToPlayer(playerid, 3.5, false);
			...
			if(x == pvData[i][cVeh])
			{
				...
				{
					new str[128];
					format(str, sizeof(str), "Safe Balance: %d\n\nPlease enter how much Besi you wish to withdraw from the safe:", vsData[carid][tBesi]);
					ShowPlayerDialog(playerid, BARANG_AMBILBESI, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
					return 1;
				}

				if(amount < 1 || amount > vsData[carid][tBesi])
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much Besi you wish to withdraw from the safe:", vsData[carid][tBesi]);
					ShowPlayerDialog(playerid, BARANG_AMBILBESI, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				...
				else if(pData[playerid][pBesi] > 42)
				{
				     Error(playerid, "cuman bisa ambil 42 Besi");
				     return 1;
				}
				vsData[carid][tBesi] -= amount;
				pData[playerid][pBesi] += amount;
	...
	if(dialogid == BARANG_MASUKINBESI)
	{
		if(response)
		{
			new x = GetNearestVehicleToPlayer(playerid, 3.5, false);
			...
			if(x == pvData[i][cVeh])
			{
				new carid = pvData[i][cVeh];
				new amount = strval(inputtext);
				new total = vsData[carid][tBesi] + amount;
				if(isnull(inputtext))
				{
					new str[128];
					format(str, sizeof(str), "Safe Balance: %d\n\nPlease enter how much Besi you wish to deposit into the safe:", vsData[carid][tBesi]);
					ShowPlayerDialog(playerid, BARANG_MASUKINBESI, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
					return 1;
				}
				if(amount < 1 || amount > pData[playerid][pBesi] || total > 126)
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much Besi you wish to deposit into the safe:", vsData[carid][tBesi]);
					ShowPlayerDialog(playerid, BARANG_MASUKINBESI, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
	...
	if(dialogid == BARANG_EMAS)
	{
		if(response)
		{
			switch (listitem)
			{
				...
				{
					new str[128];
					format(str, sizeof(str), "Please enter how much Emas you wish to withdraw from the safe:");
					ShowPlayerDialog(playerid, BARANG_AMBILEMAS, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				}
				case 1:
				{
					new str[128];
					format(str, sizeof(str), "Please enter how much Emas you wish to deposit into the safe:");
					ShowPlayerDialog(playerid, BARANG_MASUKINEMAS, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
				}
			}
		}
		return 1;
	}
	if(dialogid == BARANG_AMBILEMAS)
	{
		if(response)
		{
			new x = GetNearestVehicleToPlayer(playerid, 3.5, false);
			...
			if(x == pvData[i][cVeh])
			{
				...
				{
					new str[128];
					format(str, sizeof(str), "Safe Balance: %d\n\nPlease enter how much Emas you wish to withdraw from the safe:", vsData[carid][tEmas]);
					ShowPlayerDialog(playerid, BARANG_AMBILEMAS, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
					return 1;
				}

				if(amount < 1 || amount > vsData[carid][tEmas])
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much Emas you wish to withdraw from the safe:", vsData[carid][tEmas]);
					ShowPlayerDialog(playerid, BARANG_AMBILEMAS, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				...
				else if(pData[playerid][pEmas] > 21)
				{
				     Error(playerid, "cuman bisa ambil 21 Emas");
				     return 1;
				}
				vsData[carid][tEmas] -= amount;
				pData[playerid][pEmas] += amount;
	...
	if(dialogid == BARANG_MASUKINEMAS)
	{
		if(response)
		{
			new x = GetNearestVehicleToPlayer(playerid, 3.5, false);
			...
			if(x == pvData[i][cVeh])
			{
				new carid = pvData[i][cVeh];
				new amount = strval(inputtext);
				new total = vsData[carid][tEmas] + amount;
				if(isnull(inputtext))
				{
					new str[128];
					format(str, sizeof(str), "Safe Balance: %d\n\nPlease enter how much Emas you wish to deposit into the safe:", vsData[carid][tEmas]);
					ShowPlayerDialog(playerid, BARANG_MASUKINEMAS, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
					return 1;
				}
				if(amount < 1 || amount > pData[playerid][pEmas] || total > 63)
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much Emas you wish to deposit into the safe:", vsData[carid][tEmas]);
					ShowPlayerDialog(playerid, BARANG_MASUKINEMAS, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
	...
	if(dialogid == BARANG_BERLIAN)
	{
		if(response)
		{
			switch (listitem)
			{
				...
				{
					new str[128];
					format(str, sizeof(str), "Please enter how much Berlian you wish to withdraw from the safe:");
					ShowPlayerDialog(playerid, BARANG_AMBILBERLIAN, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				}
				case 1:
				{
					new str[128];
					format(str, sizeof(str), "Please enter how much Berlian you wish to deposit into the safe:");
					ShowPlayerDialog(playerid, BARANG_MASUKINBERLIAN, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
				}
			}
		}
		return 1;
	}
	if(dialogid == BARANG_AMBILBERLIAN)
	{
		if(response)
		{
			new x = GetNearestVehicleToPlayer(playerid, 3.5, false);
			...
			if(x == pvData[i][cVeh])
			{
				...
				{
					new str[128];
					format(str, sizeof(str), "Safe Balance: %d\n\nPlease enter how much Berlian you wish to withdraw from the safe:", vsData[carid][tBerlian]);
					ShowPlayerDialog(playerid, BARANG_AMBILBERLIAN, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
					return 1;
				}

				if(amount < 1 || amount > vsData[carid][tBerlian])
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much Berlian you wish to withdraw from the safe:", vsData[carid][tBerlian]);
					ShowPlayerDialog(playerid, BARANG_AMBILBERLIAN, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				...
				else if(pData[playerid][pBerlian] > 5)
				{
				     Error(playerid, "cuman bisa ambil 5 Berlian");
				     return 1;
				}
				vsData[carid][tBerlian] -= amount;
				pData[playerid][pBerlian] += amount;
	...
	if(dialogid == BARANG_MASUKINBERLIAN)
	{
		if(response)
		{
			new x = GetNearestVehicleToPlayer(playerid, 3.5, false);
			...
			if(x == pvData[i][cVeh])
			{
				new carid = pvData[i][cVeh];
				new amount = strval(inputtext);
				new total = vsData[carid][tBerlian] + amount;
				if(isnull(inputtext))
				{
					new str[128];
					format(str, sizeof(str), "Safe Balance: %d\n\nPlease enter how much Berlian you wish to deposit into the safe:", vsData[carid][tBerlian]);
					ShowPlayerDialog(playerid, BARANG_MASUKINBERLIAN, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
					return 1;
				}
				if(amount < 1 || amount > pData[playerid][pBerlian] || total > 5)
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much Berlian you wish to deposit into the safe:", vsData[carid][tBerlian]);
					ShowPlayerDialog(playerid, BARANG_MASUKINBERLIAN, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
	...
	if(dialogid == TRUNK_DEPOSITCOMP)
	{
		if(response)
		{
			...
			if(x == pvData[i][cVeh])
			{
				...
				if(amount < 1 || amount > pData[playerid][pComponent] || total > 500)
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much component you wish to deposit into the safe:", vsData[carid][tComp]);
					ShowPlayerDialog(playerid, TRUNK_DEPOSITCOMP, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
	...
	if(dialogid == TRUNK_DEPOSITMATS)
	{
		if(response)
		{
			...
			if(x == pvData[i][cVeh])
			{
				...
				if(amount < 1 || amount > pData[playerid][pMaterial] || total > 1000)
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much material you wish to deposit into the safe:", vsData[carid][tMats]);
					ShowPlayerDialog(playerid, TRUNK_DEPOSITMATS, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
	...
	if(dialogid == TRUNK_BORAX)
	{
		if(response)
		{
			switch (listitem)
			{
				...
				{
					new str[128];
					format(str, sizeof(str), "Please enter how much borax you wish to withdraw from the safe:");
					ShowPlayerDialog(playerid, TRUNK_WITHDRAWBOR, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				}
				case 1:
				{
					new str[128];
					format(str, sizeof(str), "Please enter how much borax you wish to deposit into the safe:");
					ShowPlayerDialog(playerid, TRUNK_DEPOSITBOR, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
				}
			}
		}
		return 1;
	}
	if(dialogid == TRUNK_WITHDRAWBOR)
	{
		if(response)
		{
			new x = GetNearestVehicleToPlayer(playerid, 3.5, false);
			...
			if(x == pvData[i][cVeh])
			{
				...
				{
					new str[128];
					format(str, sizeof(str), "Safe Balance: %d\n\nPlease enter how much borax you wish to withdraw from the safe:", vsData[carid][tBor]);
					ShowPlayerDialog(playerid, TRUNK_WITHDRAWBOR, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
					return 1;
				}

				if(amount < 1 || amount > vsData[carid][tBor])
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much borax you wish to withdraw from the safe:", vsData[carid][tBor]);
					ShowPlayerDialog(playerid, TRUNK_WITHDRAWBOR, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				...
				else if(pData[playerid][pBorax] > 120)
				{
				     Error(playerid, "cuman bisa ambil 120 borax");
				     return 1;
				}
				vsData[carid][tBor] -= amount;
				pData[playerid][pBorax] += amount;
	...
	if(dialogid == TRUNK_DEPOSITBOR)
	{
		if(response)
		{
			...
			if(x == pvData[i][cVeh])
			{
				new carid = pvData[i][cVeh];
				new amount = strval(inputtext);
				new total = vsData[carid][tBor] + amount;
				if(isnull(inputtext))
				{
					new str[128];
					format(str, sizeof(str), "Safe Balance: %d\n\nPlease enter how much borax you wish to deposit into the safe:", vsData[carid][tBor]);
					ShowPlayerDialog(playerid, TRUNK_DEPOSITBOR, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
					return 1;
				}
				if(amount < 1 || amount > pData[playerid][pBorax] || total > 1200)
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much borax you wish to deposit into the safe:", vsData[carid][tBor]);
					ShowPlayerDialog(playerid, TRUNK_DEPOSITBOR, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
	...
	if(dialogid == TRUNK_OBOR)
	{
		if(response)
		{
			switch (listitem)
			{
				...
				{
					new str[128];
					format(str, sizeof(str), "Please enter how much olahan borax you wish to withdraw from the safe:");
					ShowPlayerDialog(playerid, TRUNK_WITHDRAWOBOR, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				}
				case 1:
				{
					new str[128];
					format(str, sizeof(str), "Please enter how much olahan borax you wish to deposit into the safe:");
					ShowPlayerDialog(playerid, TRUNK_DEPOSITOBOR, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
				}
			}
		}
		return 1;
	}
	if(dialogid == TRUNK_WITHDRAWOBOR)
	{
		if(response)
		{
			new x = GetNearestVehicleToPlayer(playerid, 3.5, false);
			...
			if(x == pvData[i][cVeh])
			{
				...
				{
					new str[128];
					format(str, sizeof(str), "Safe Balance: %d\n\nPlease enter how much olahan borax you wish to withdraw from the safe:", vsData[carid][tObor]);
					ShowPlayerDialog(playerid, TRUNK_WITHDRAWOBOR, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
					return 1;
				}

				if(amount < 1 || amount > vsData[carid][tObor])
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much olahan borax you wish to withdraw from the safe:", vsData[carid][tObor]);
					ShowPlayerDialog(playerid, TRUNK_WITHDRAWOBOR, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				...
				else if(pData[playerid][pPaketBorax] > 10)
				{
				     Error(playerid, "cuman bisa ambil 10 olahan borax");
				     return 1;
				}
				vsData[carid][tObor] -= amount;
	...
	if(dialogid == TRUNK_DEPOSITOBOR)
	{
		if(response)
		{
			...
			if(x == pvData[i][cVeh])
			{
				new carid = pvData[i][cVeh];
				new amount = strval(inputtext);
				new total = vsData[carid][tObor] + amount;
				if(isnull(inputtext))
				{
					new str[128];
					format(str, sizeof(str), "Safe Balance: %d\n\nPlease enter how much olahan borax you wish to deposit into the safe:", vsData[carid][tObor]);
					ShowPlayerDialog(playerid, TRUNK_DEPOSITOBOR, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
					return 1;
				}
				if(amount < 1 || amount > pData[playerid][pPaketBorax] || total > 200)
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much olahan borax you wish to deposit into the safe:", vsData[carid][tObor]);
					ShowPlayerDialog(playerid, TRUNK_DEPOSITOBOR, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
	...
	if(dialogid == TRUNK_SEED)
	{
		if(response)
		{
			switch (listitem)
			{
				...
				{
					new str[128];
					format(str, sizeof(str), "Please enter how much seed you wish to withdraw from the safe:");
					ShowPlayerDialog(playerid, TRUNK_WITHDRAWSEED, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				}
				case 1:
				{
					new str[128];
					format(str, sizeof(str), "Please enter how much seed you wish to deposit into the safe:");
					ShowPlayerDialog(playerid, TRUNK_DEPOSITSEED, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
				}
			}
		}
		return 1;
	}
	if(dialogid == TRUNK_WITHDRAWSEED)
	{
		if(response)
		{
			new x = GetNearestVehicleToPlayer(playerid, 3.5, false);
			...
			if(x == pvData[i][cVeh])
			{
				...
				{
					new str[128];
					format(str, sizeof(str), "Safe Balance: %d\n\nPlease enter how much seed you wish to withdraw from the safe:", vsData[carid][tSeed]);
					ShowPlayerDialog(playerid, TRUNK_WITHDRAWSEED, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
					return 1;
				}

				if(amount < 1 || amount > vsData[carid][tSeed])
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much seed you wish to withdraw from the safe:", vsData[carid][tSeed]);
					ShowPlayerDialog(playerid, TRUNK_WITHDRAWSEED, DIALOG_STYLE_INPUT, "Withdraw from safe", str, "Withdraw", "Back");
				...
				else if(pData[playerid][pSeed] > 500)
				{
				     Error(playerid, "cuman bisa ambil 500 seed");
				     return 1;
				}
				vsData[carid][tSeed] -= amount;
				pData[playerid][pSeed] += amount;
	...
	if(dialogid == TRUNK_DEPOSITSEED)
	{
		if(response)
		{
			...
			if(x == pvData[i][cVeh])
			{
				new carid = pvData[i][cVeh];
				new amount = strval(inputtext);
				new total = vsData[carid][tSeed] + amount;
				if(isnull(inputtext))
				{
					new str[128];
					format(str, sizeof(str), "Safe Balance: %d\n\nPlease enter how much seed you wish to deposit into the safe:", vsData[carid][tSeed]);
					ShowPlayerDialog(playerid, TRUNK_DEPOSITSEED, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
					return 1;
				}
				if(amount < 1 || amount > pData[playerid][pSeed] || total > 500)
				{
					new str[128];
					format(str, sizeof(str), "Error: Insufficient funds.\n\nSafe Balance: %d\n\nPlease enter how much seed you wish to deposit into the safe:", vsData[carid][tSeed]);
					ShowPlayerDialog(playerid, TRUNK_DEPOSITSEED, DIALOG_STYLE_INPUT, "Deposit into safe", str, "Deposit", "Back");
	...
	if(dialogid == DIALOG_SERVERMONEY_STORAGE)
	{
		...
		else 
		{
			...
			ShowPlayerDialog(playerid, DIALOG_SERVERMONEY, DIALOG_STYLE_MSGBOX, "HOPIXEL Money", lstr, "Manage", "Close");
		}
	}
	...
	if(dialogid == DIALOG_TRACK)
	{
 		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					return cmd_tr(playerid, "ph");
				}
				case 1:
				{
					return cmd_tr(playerid, "bis");
				}
				case 2:
				{
					return cmd_tr(playerid, "house");
				}
			}
			return 1;
		}
	...
	if(dialogid == DIALOG_INFO_BIS)
	{
 		if(response)
		{
			...
			pData[playerid][pCheckingBis] = 1;
		 	pData[playerid][pActivity] = SetTimerEx("CheckingBis", 5000, false, "i", playerid);
			ShowProgressbar(playerid, "Check", 5);
		}
		return 1;
	}
	if(dialogid == DIALOG_INFO_HOUSE)
	{
 		if(response)
		{
			...
			pData[playerid][pCheckingBis] = 1;
		 	pData[playerid][pActivity] = SetTimerEx("CheckingHouse", 5000, false, "i", playerid);
			ShowProgressbar(playerid, "Check", 5);
		}
		return 1;
	}
		...
		if (response)
		{
		    switch (listitem)
		    {
				...
				//	new ammo = GetPlayerAmmoEx(playerid);
					if (!senjata)   return Error(playerid, "Clip DE Bodoh.");
					if (!weaponid)	return Error(playerid, "Pegang DE nya ngab.");
					if(pData[playerid][pClipA] < 1)
						return Error(playerid, "Clip Abis tod.");

					pData[playerid][pClipA] -= 1;
					ApplyAnimation(playerid,"COLT45","colt45_reload",4.0, 1, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealer] = SetTimerEx("UseClip", 3000, false, "idd", playerid, WEAPON_DEAGLE, 50);
					ShowProgressbar(playerid, "Reload", 3);
				}
				case 1:
				{
					...
					new weaponid = GetPlayerWeaponEx(playerid);
					//new ammo = GetPlayerAmmoEx(playerid);
					if (!senjata)   return Error(playerid, "Clip SG Bodoh.");
					if (!weaponid)	return Error(playerid, "Pegang SG nya ngab.");
					if(pData[playerid][pClipB] < 1)
						return Error(playerid, "Clip Abis tod.");

					pData[playerid][pClipB] -= 1;
					ApplyAnimation(playerid,"COLT45","colt45_reload",4.0, 1, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealer] = SetTimerEx("UseClip", 3000, false, "idd", playerid, WEAPON_SHOTGUN, 30);
					ShowProgressbar(playerid, "Reload", 3);
				}
				case 2:
				{
					...
					new weaponid = GetPlayerWeaponEx(playerid);
					//new ammo = GetPlayerAmmoEx(playerid);
					if (!senjata)   return Error(playerid, "Clip AK47 Bodoh.");
					if (!weaponid)	return Error(playerid, "Pegang AK47 nya ngab.");
					if(pData[playerid][pClipC] < 1)
						return Error(playerid, "Clip Abis tod.");

					pData[playerid][pClipC] -= 1;
					ApplyAnimation(playerid,"COLT45","colt45_reload",4.0, 1, 0, 0, 0, 0, 1);
					pData[playerid][pArmsDealer] = SetTimerEx("UseClip", 3000, false, "idd", playerid, WEAPON_AK47, 70);
					ShowProgressbar(playerid, "Reload", 3);
				}
			}
		}
	...
	if(dialogid == DIALOG_BELICLIP)
    {
        if(response)
        {
            switch(listitem)
            {
                case 0:
                {
                    if(pData[playerid][pRedMoney] < 10000) return Error(playerid, "Uang merah kurang (butuh 10000$).");
					if(pData[playerid][pClipA] > 10)    return Error(playerid, "Clip Mu sudah penuh.");
					{
				...
				{
				    if(pData[playerid][pRedMoney] < 10000) return Error(playerid, "Uang merah kurang (butuh 10000$).");
					if(pData[playerid][pClipB] > 10)    return Error(playerid, "Clip Mu sudah penuh.");
					{
						pData[playerid][pRedMoney] -= 10000;
					    pData[playerid][pClipB] += 1;
					}
				...
				{
				    if(pData[playerid][pRedMoney] < 10000) return Error(playerid, "Uang merah kurang (butuh 10000$).");
					if(pData[playerid][pClipC] > 10)    return Error(playerid, "Clip Mu sudah penuh.");
					{
						pData[playerid][pRedMoney] -= 10000;
					    pData[playerid][pClipC] += 1;
					}
		...
		if (response)
		{
		    switch (listitem)
		    {
		        case 0:
		        {
					...
					InfoTD_MSG(playerid, 3000, "Restore +40 Hunger");
					ApplyAnimation(playerid,"SMOKING","M_smkstnd_loop",2.1,0,0,0,0,0);
				}
				case 1:
				{
					if(pData[playerid][pBurger] < 1)
						return Error(playerid, "Anda tidak memiliki Siomay.");

					pData[playerid][pBurger]--;
					pData[playerid][pHunger] += 40;
					Info(playerid, "Anda telah berhasil memakan Siomay.");
					InfoTD_MSG(playerid, 3000, "Restore +40 Hunger");
					ApplyAnimation(playerid,"SMOKING","M_smkstnd_loop",2.1,0,0,0,0,0);
				}
				case 2:
				{
					...
					Info(playerid, "Anda telah berhasil memakan Snack.");
				...
				case 3:
				{
					if(pData[playerid][pAyam] < 1)
						return Error(playerid, "Anda tidak memiliki Es Susu.");

					pData[playerid][pAyam]--;
					pData[playerid][pEnergy] += 50;
					Info(playerid, "Anda telah berhasil meminum Es Susu.");
					InfoTD_MSG(playerid, 3000, "Restore +50 Energy");
					ApplyAnimation(playerid,"SMOKING","M_smkstnd_loop",2.1,0,0,0,0,0);
				}
				case 4:
				{
					if(pData[playerid][pMineral] < 1)
						return Error(playerid, "Anda tidak memiliki Jus Jeruk.");

					pData[playerid][pMineral]--;
					pData[playerid][pEnergy] += 50;
					Info(playerid, "Anda telah berhasil meminum Jus Jeruk.");
					InfoTD_MSG(playerid, 3000, "Restore +50 Energy");
					ApplyAnimation(playerid,"SMOKING","M_smkstnd_loop",2.1,0,0,0,0,0);
				}
				case 5:
