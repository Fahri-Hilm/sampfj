	...
	if(rows)
  	{
		...
		while(i < rows)
		{
			format(str, sizeof(str), "[ID: %d]\n"AQUA"%s"YELLOW_E"\nPress [Y] to Park/Pickup Vehicle", id, GetLocation(ppData[id][parkX], ppData[id][parkY], ppData[id][parkZ]));
...
{
	Info(playerid, "/creategarage, /setgpos, /removegarage, /gotogarage");
	return 1;
}
...
CMD:setgpos(playerid, params[])
{
    if(pData[playerid][pAdmin] < 5)
		return PermissionError(playerid);

	new id;
	if(sscanf(params, "i", id)) return Usage(playerid, "/setgpos [id]");
